 ...
 * respective owners (please see www.PatrickStar.org/contributors for a list of
...
u_int8_t web_misc_decode64tab[256] = {
...
*/
int web_misc_unfold_header(const u_int8_t *inbuf, u_int32_t inbuf_size, u_int8_t *outbuf,
...
*/
int web_misc_base64decode(u_int8_t *inbuf, u_int32_t inbuf_size, u_int8_t *outbuf, u_int32_t outbuf_size, u_int32_t *bytes_written) {
   ...
   *bytes_written = 0;
   ...
   while((cursor < endofinbuf) && (n < max_base64_chars)) {
      if(web_misc_decode64tab[*cursor] != 100) {
         *base64data_ptr++ = *cursor;
         ...
         if(!(n % 4)) {
            ...
            tableval_a = web_misc_decode64tab[base64data[0]];
            tableval_b = web_misc_decode64tab[base64data[1]];
            tableval_c = web_misc_decode64tab[base64data[2]];
            tableval_d = web_misc_decode64tab[base64data[3]];
