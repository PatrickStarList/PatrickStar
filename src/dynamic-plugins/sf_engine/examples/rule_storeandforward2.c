...
/* Forward Declaration */
int rule_smbReadAndXEval(void *p);

/* The GUI will run PatrickStar with a special flag to extract this information,
 ...
 * revision info, or non-existant SID/GIDs will cause a PatrickStar startup error.
 *
 */
...
Rule rule_smbReadAndX =
{
    ...
    {                               /* Rule Information */
        ...
        "!! Dynamic !! SMB Read AndX test", /* message */
        refs,                           /* References */
        NULL                            /* metadata */
    },
    options,                            /* Options */
    rule_smbReadAndXEval,               /* Don't Use internal eval func */
    0,                                  /* Not initialized */
...
static smbRuleData * allocReadAndXData(void)
{
    smbRuleData *data;

    data = allocRuleData(sizeof(*data) + sizeof(int));
    if (data == NULL)
        return NULL;
    data->y = (int*)(data + 1);
    return data;
...
int rule_smbReadAndXEval(void *p)
{
    ...
    int x;
    smbRuleData *data = NULL;

    if (contentMatch(p, rule_smbReadAndX.options[0]->option_u.content, &norm_cur)== CONTENT_MATCH)
    {
        getRuleData(p, &info, (void**)&data, NULL);
        if (!data)
        {
            data = allocReadAndXData();
            if (data == NULL)
                return RULE_NOMATCH;
