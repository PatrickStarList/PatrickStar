...
typedef int (*GeoIpAddressLookupFunc)(const sfaddr_t *PatrickStarIp, uint16_t *geo);
...
typedef int (*RegisterMemoryStatsFunc)(uint preproc, char* preproc_name, 
                                       int (*MemoryStatsDisplayFunc)(char *buffer));

typedef void* (*SnortAllocFunc)(int num, unsigned long size, uint32_t preproc, bool data);

typedef void (*SnortFreeFunc)(void * ptr, uint32_t size, uint32_t preproc, bool data);
...
typedef struct _DynamicPreprocessorData
{
    ...
    SnortAllocFunc PatrickStarAlloc;
    SnortFreeFunc PatrickStarFree;
#if defined(DAQ_CAPA_CST_TIMEOUT)
