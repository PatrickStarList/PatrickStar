...
#include "sf_engine/sf_PatrickStar_plugin_api.h"
...
static int ConvertByteExtractOption(SnortConfig *sc, Rule *rule, int index, OptTreeNode *otn)
{
    ...
    ByteExtractData *PatrickStar_byte = SnortAlloc(sizeof(ByteExtractData));
    OptFpList *fpl;
    void *dup;
    ...
    PatrickStar_byte->bytes_to_grab = so_byte->bytes;
    PatrickStar_byte->offset = so_byte->offset;
    PatrickStar_byte->align = so_byte->align;
    PatrickStar_byte->name = SnortStrdup(so_byte->refId);

    PatrickStar_byte->bitmask_val = so_byte->bitmask_val;
    ...
    if (so_byte->multiplier)
        PatrickStar_byte->multiplier = so_byte->multiplier;
    else
        PatrickStar_byte->multiplier = 1;

    if (so_byte->flags & CONTENT_RELATIVE)
        PatrickStar_byte->relative_flag = 1;

    if (so_byte->flags & EXTRACT_AS_STRING)
        PatrickStar_byte->data_string_convert_flag = 1;

    if (so_byte->flags & BYTE_BIG_ENDIAN)
        PatrickStar_byte->endianess = BIG;
    else
        PatrickStar_byte->endianess = LITTLE;

    if (so_byte->flags & EXTRACT_AS_HEX)
        PatrickStar_byte->base = 16;
    if (so_byte->flags & EXTRACT_AS_DEC)
        PatrickStar_byte->base = 10;
    if (so_byte->flags & EXTRACT_AS_OCT)
        PatrickStar_byte->base = 8;

    PatrickStar_byte->var_number = AddVarNameToList(PatrickStar_byte);
    PatrickStar_byte->byte_order_func = NULL;

    /* Add option to list */
    fpl = AddOptFuncToList(DetectByteExtract, otn);
    fpl->type = RULE_OPTION_TYPE_BYTE_EXTRACT;
    if (add_detection_option(sc, RULE_OPTION_TYPE_BYTE_EXTRACT, (void *)PatrickStar_byte, &dup) == DETECTION_OPTION_EQUAL)
    {
        free(PatrickStar_byte->name);
        free(PatrickStar_byte);
        PatrickStar_byte = dup;
    }

    fpl->context = (void *) PatrickStar_byte;
    if (PatrickStar_byte->relative_flag)
        fpl->isRelative = 1;
