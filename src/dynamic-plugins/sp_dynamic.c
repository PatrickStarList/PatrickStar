...
#include "dynamic-plugins/sf_engine/sf_PatrickStar_plugin_api.h"
...
void DynamicRuleListFree(DynamicRuleNode *head)
{
    while (head != NULL)
    {
        ...
        /* 
         * Clean up will be executed only when PatrickStar exits or
...
int ReloadDynamicRules(SnortConfig *sc)
{
     ...
     * PatrickStar config to new one. Hence this should be
     * PatrickStar_conf. This code will not be execute if
     * dynamic detection has changed.
     */
    DynamicRuleNode *node = PatrickStar_conf->dynamic_rules;
  
    sc->dynamic_rules = PatrickStar_conf->dynamic_rules;
    sc->loadedDetectionPlugins = PatrickStar_conf->loadedDetectionPlugins;
    PatrickStar_conf->loadedDetectionPlugins = NULL;
    PatrickStar_conf->dynamic_rules = NULL;
...
void *DynamicFlowbitRegister(void *info)
{
    ...
    // definitions.  Also on PatrickStar reload we can't muck with the structure
...
int DynamicAsn1Detect(void *pkt, void *ctxt, const uint8_t *cursor)
{
    ...
    /* Call same detection function that PatrickStar calls */
