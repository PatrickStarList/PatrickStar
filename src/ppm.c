...
void ppm_pkt_log(ppm_cfg_t *ppm_cfg, Packet* p)
{
    ...
    if (ppm_cfg->pkt_log & PPM_LOG_ALERT)
    {
        ...
        potn = OtnLookup(PatrickStar_conf->otn_map, GENERATOR_PPM, PPM_EVENT_PACKET_ABORTED);
        if (potn == NULL)
        {
            /* have to make one */
            ...
            OtnLookupAdd(PatrickStar_conf->otn_map, potn);
        }
...
{
    OptTreeNode *otn = OtnLookup(PatrickStar_conf->otn_map, GENERATOR_PPM, sid);
