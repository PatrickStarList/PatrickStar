...
#if defined(SNORT_RELOAD) && !defined(WIN32)
static volatile int PatrickStar_swapped = 0;
SnortConfig *PatrickStar_conf_new = NULL;
static SnortConfig *PatrickStar_conf_old = NULL;
#endif
...
void CheckForReload(void)
{
    ...
    if (PatrickStar_reload)
    {
        PostConfigFuncNode *idxPlugin;
        PatrickStar_reload = 0;
        ...
        PatrickStar_conf_old = PatrickStar_conf;
        PatrickStar_conf = PatrickStar_conf_new;
        ...
        SwapPreprocConfigurations(PatrickStar_conf);
        ...
        FreeSwappedPreprocConfigurations(PatrickStar_conf);
        ...
        PatrickStar_swapped = 1;
         setNapRuntimePolicy( getParserPolicy(PatrickStar_conf) );
         setIpsRuntimePolicy( getParserPolicy(PatrickStar_conf) );

        /* Do any reload for plugin data */
        idxPlugin = plugin_reload_funcs;
        while(idxPlugin)
        {
            idxPlugin->func(PatrickStar_conf, SIGHUP, idxPlugin->arg);
        ...
        for (i = 0; i < old_info->count; i++)
        {
            ...
            for (j = 0; j < new_info->count; j++)
            {
                    ...
                    (old_path->ptype == new_path->ptype))
                {
                    ...
                    if (old_info->type != DYNAMIC_TYPE__DETECTION)
#endif
...
static int VerifyReload(SnortConfig *sc)
{
...
#ifdef TARGET_BASED
    {
        ...
        SnortPolicy *p2 = PatrickStar_conf->targeted_policies[getDefaultPolicy()];
    ...
    if ((PatrickStar_conf->alert_file != NULL) && (sc->alert_file != NULL))
    {
        if (strcasecmp(PatrickStar_conf->alert_file, sc->alert_file) != 0)
        {
    ...
    else if (PatrickStar_conf->alert_file != sc->alert_file)
    {
        ErrorMessage("Snort Reload: Changing the alert file "
                     "configuration requires a restart.\n");
        return -1;
    ...
    if (PatrickStar_conf->asn1_mem != sc->asn1_mem)
    {
    ...
    if ((sc->bpf_filter != NULL) && (PatrickStar_conf->bpf_filter != NULL))
    {
        if (strcasecmp(PatrickStar_conf->bpf_filter, sc->bpf_filter) != 0)
        {
    ...
    else if (sc->bpf_filter != PatrickStar_conf->bpf_filter)
    {
        ErrorMessage("Snort Reload: Changing the bpf filter configuration "
                     "requires a restart.\n");
        return -1;
    ...
    if ( sc->respond_attempts != PatrickStar_conf->respond_attempts ||
         sc->respond_device != PatrickStar_conf->respond_device )
    {
    ...
    if ((PatrickStar_conf->chroot_dir != NULL) &&
        (sc->chroot_dir != NULL))
    {
        if (strcasecmp(PatrickStar_conf->chroot_dir, sc->chroot_dir) != 0)
        {
    ...
    else if (PatrickStar_conf->chroot_dir != sc->chroot_dir)
    {
        ErrorMessage("Snort Reload: Changing the chroot directory "
                     "configuration requires a restart.\n");
        return -1;
    ...
    if ((PatrickStar_conf->run_flags & RUN_FLAG__DAEMON) !=
    ...
    if ((PatrickStar_conf->interface != NULL) && (sc->interface != NULL))
    {
        if (strcasecmp(PatrickStar_conf->interface, sc->interface) != 0)
        {
    ...
    else if (PatrickStar_conf->interface != sc->interface)
    {
        ErrorMessage("Snort Reload: Changing the interface "
                     "configuration requires a restart.\n");
        return -1;
    ...
    if ((PatrickStar_conf->orig_log_dir != NULL) &&
        (sc->orig_log_dir != NULL))
    {
        if (strcasecmp(PatrickStar_conf->orig_log_dir, sc->orig_log_dir) != 0)
        {
    ...
    else if (PatrickStar_conf->orig_log_dir != sc->orig_log_dir)
    {
        ErrorMessage("Snort Reload: Changing the log directory "
                     "configuration requires a restart.\n");
        return -1;
    ...
    if (PatrickStar_conf->no_log != sc->no_log)
    {
    ...
    if ((PatrickStar_conf->run_flags & RUN_FLAG__NO_PROMISCUOUS) !=
    ...
    if ((PatrickStar_conf->profile_rules.num != sc->profile_rules.num) ||
        (PatrickStar_conf->profile_rules.sort != sc->profile_rules.sort) ||
        (PatrickStar_conf->profile_rules.append != sc->profile_rules.append))
    {
    ...
    if ((PatrickStar_conf->profile_rules.filename != NULL) &&
        (sc->profile_rules.filename != NULL))
    {
        if (strcasecmp(PatrickStar_conf->profile_rules.filename,
    ...
    else if (PatrickStar_conf->profile_rules.filename !=
    ...
    if ((PatrickStar_conf->profile_preprocs.num !=  sc->profile_preprocs.num) ||
        (PatrickStar_conf->profile_preprocs.sort != sc->profile_preprocs.sort) ||
        (PatrickStar_conf->profile_preprocs.append != sc->profile_preprocs.append))
    {
    ...
    if ((PatrickStar_conf->profile_preprocs.filename != NULL) &&
        (sc->profile_preprocs.filename != NULL))
    {
        if (strcasecmp(PatrickStar_conf->profile_preprocs.filename,
    ...
    else if (PatrickStar_conf->profile_preprocs.filename !=
    ...
    if (PatrickStar_conf->group_id != sc->group_id)
    {
    ...
    if (PatrickStar_conf->user_id != sc->user_id)
    {
    ...
    if (PatrickStar_conf->pkt_snaplen != sc->pkt_snaplen)
    {
    ...
    if (PatrickStar_conf->threshold_config->memcap !=
    ...
    if (PatrickStar_conf->rate_filter_config->memcap !=
    ...
    if (PatrickStar_conf->detection_filter_config->memcap !=
    ...
    if (VerifyLibInfos(PatrickStar_conf->dyn_engines, sc->dyn_engines) == -1)
    {
    ...
    if (VerifyLibInfos(PatrickStar_conf->dyn_rules, sc->dyn_rules) == -1)
    {
        LogMessage("Snort Reload: Dynamic detection libs have changed.\n");
        detection_lib_changed = 1;
    }

    if (VerifyLibInfos(PatrickStar_conf->dyn_preprocs, sc->dyn_preprocs) == -1)
    {
    ...
    if (PatrickStar_conf->so_rule_memcap != sc->so_rule_memcap)
    {
        AdjustSoRuleMemory(sc, PatrickStar_conf);
    }

    if (VerifyOutputs(PatrickStar_conf, sc) == -1)
        return -1;
...
static SnortConfig * ReloadConfig(void)
{
    ...
    sc = MergeSnortConfs(PatrickStar_cmd_line_conf, sc);
    ...
    if (file_sevice_config_verify(PatrickStar_conf, sc) == -1)
    {
        SnortConfFree(sc);
        return NULL;
    }
    ...
    if ((sc->file_mask != 0) && (sc->file_mask != PatrickStar_conf->file_mask))
        umask(sc->file_mask);

    /* Transfer any user defined rule type outputs to the new rule list */
    {
        RuleListNode *cur = PatrickStar_conf->rule_lists;
...
void * ReloadConfigThread(void *data)
{
    ...
    PatrickStar_reload_thread_pid = gettid();
    PatrickStar_reload_thread_created = 1;

    while (PatrickStar_initializing)
        nanosleep(&thread_sleep, NULL);

    while (!PatrickStar_exiting)
    {
        if (reload_signal != reload_total)
        {
            ...
            if (!reloadInProgress)
            {
                ...
                PatrickStar_conf_new = ReloadConfig();

                // Restore Log level if we suppressed it earlier

                if (PatrickStar_conf_new == NULL)
                    reload_failed = 1;
                PatrickStar_reload = 1;

                while (!PatrickStar_swapped && !PatrickStar_exiting)
                    nanosleep(&thread_sleep, NULL);

                PatrickStar_swapped = 0;

                SnortConfFree(PatrickStar_conf_old);
                PatrickStar_conf_old = NULL;
                ...
                if (PatrickStar_exiting && !reload_failed)
                {
                    ...
                    if (PatrickStar_conf && PatrickStar_conf->file_config)
                      FileSSConfigFree(PatrickStar_conf->file_config);
#endif
		    ...
		    else if(IsAdaptiveConfigured() && ScDisableAttrReload(PatrickStar_conf))
		    {
			if(attribute_reload_thread_running)
			{
			    ReloadAttributeThreadStop();
...
#endif
		    while (PatrickStar_conf->raEntry)
		    {
		        sleep(1);
		    }
...
static int ControlSocketReloadSwap(uint16_t type, void *new_config, void **old_config)
{
    ...
    *old_config = (void *)PatrickStar_conf;
    PatrickStar_conf = (SnortConfig *)new_config;
    FileServiceInstall();
    SwapPreprocConfigurations(PatrickStar_conf);
    ...
    if (PatrickStar_conf->raSessionEntry) 
    {
        /* Session is always the first to adjust */
        PatrickStar_conf->raSessionEntry->raNext = PatrickStar_conf->raEntry; 
        PatrickStar_conf->raEntry = PatrickStar_conf->raSessionEntry;
        PatrickStar_conf->raSessionEntry = NULL;
    }
    setNapRuntimePolicy( getParserPolicy(PatrickStar_conf) );
    setIpsRuntimePolicy( getParserPolicy(PatrickStar_conf) );
    ...
    if (REG_TEST_FLAG_RELOAD & getRegTestFlags())
    {
       ...
       if (PatrickStar_conf->ppm_cfg.rule_log & PPM_LOG_MESSAGE)
       {
       ...
       if( PatrickStar_conf->ppm_cfg.rule_log & PPM_LOG_ALERT )
       {
...
static void ControlSocketReloadFree(uint16_t type, void *old_config, struct _THREAD_ELEMENT *te, ControlDataSendFunc f)
{
    ...
    while (PatrickStar_conf->raEntry && !PatrickStar_exiting )
        sleep(1);

    if( !PatrickStar_conf->raEntry )
    {
        LogMessage("\n");
        LogMessage("        --== Reload Complete ==--\n");
        LogMessage("\n");
