    ...
    Packet* p)
{
    ...
    if ((PatrickStar_conf == NULL) || (PatrickStar_conf->rate_filter_config == NULL))
    {
    ...
    if ( EventIsInternal(gid) )
    {
        ...
        if ( p->packet_flags & PKT_FROM_SERVER )
        {
            return SFRF_TestThreshold(
                PatrickStar_conf->rate_filter_config, gid, sid, dip, sip,
                p->pkth->ts.tv_sec, SFRF_COUNT_INCREMENT);
        }
    }

    return SFRF_TestThreshold(
        PatrickStar_conf->rate_filter_config, gid, sid, sip, dip,
        p->pkth->ts.tv_sec, SFRF_COUNT_INCREMENT);
}
