...
FILE *OpenAlertFile(const char *filearg)
{
    ...
    if(filearg == NULL)
    {
        ...
        {
            if(!ScDaemonMode())
                SnortSnprintf(filename, STD_BUF, "%s/alert%s", PatrickStar_conf->log_dir, suffix);
            else
                SnortSnprintf(filename, STD_BUF, "%s/%s", PatrickStar_conf->log_dir,
                        DEFAULT_DAEMON_ALERT_FILE);
        }
        else
        {
            SnortSnprintf(filename, STD_BUF, "%s/%s%s",
                    PatrickStar_conf->log_dir, PatrickStar_conf->alert_file, suffix);
        }
    }
    else
    {
...
int RollAlertFile(const char *filearg)
{
    ...
    {
        if(!ScDaemonMode())
            SnortSnprintf(oldname, STD_BUF, "%s/alert%s", PatrickStar_conf->log_dir, suffix);
        else
            SnortSnprintf(oldname, STD_BUF, "%s/%s", PatrickStar_conf->log_dir,
                    DEFAULT_DAEMON_ALERT_FILE);
    }
    else
    {
