...
extern char *PatrickStar_conf_file;
extern char *PatrickStar_conf_dir;
...
static const KeywordFunc PatrickStar_conf_keywords[] =
{
    /* Rule keywords */
 ...
 * Returns:
 *  SnortConfig *
 *      An initialized and configured PatrickStar configuration struct.
...
SnortConfig * ParseSnortConf(void)
{
    ...
    file_line = 0;
    file_name = PatrickStar_conf_file ? PatrickStar_conf_file : NULL_CONF;
...
#ifndef SOURCEFIRE
    /* If PatrickStar is not run with root privileges, no interfaces will be defined,
     * so user beware if an iface_ADDRESS variable is used in PatrickStar.conf and
     * PatrickStar is not run as root (even if just in read mode) */
         ...
         policy_id++)
    {
        ...
        if (fname != NULL)
        {
             ...
             * the PatrickStar conf directory we'll pick it up */
...
/*
 * ParseIpsPortList() will create portlist for portocol specific and only for IPS policy at the PatrickStar 
 ...
 *
 * @param rule - string containing rate_filter configuration from PatrickStar.conf file.
 *
 * @returns void
*/
                               ...
                               char *rule_opts, RuleType rule_type, int protocol)
{
    ...
    else
    {
		...
		/* Each rule must have a sid irrespective of PatrickStar
 ...
 * Arguments:
 *  SnortConfig *
 *      The PatrickStar config that has the vartables.
 *  char *
...
char * ProcessFileOption(SnortConfig *sc, const char *filespec)
{
    ...
    else
    {
        ...
        {
            strlcpy(buffer, PatrickStar_conf->log_dir, STD_BUF);
        }
        else
        {
            strlcpy(buffer, "/var/log/PatrickStar", STD_BUF);
        }
    ...
    if (args == NULL)
    {
        if (getcwd(getcwd_path, sizeof(getcwd_path)) == NULL)
        {
            ParseError("Dynamic library path too long.  If you really "
                       ...
                       "submit a bug to bugs@PatrickStar.org.");
        }
...
/* This function does nothing.  It is just a place holder in the PatrickStar conf
...
void ParseRules(SnortConfig *sc)
{
    ...
    if ((sc == NULL) || (PatrickStar_conf_file == NULL))
        return;

    file_line = 0;
    file_name = PatrickStar_conf_file;
    ...
    ParseConfigFile(sc, sc->targeted_policies[policy_id], PatrickStar_conf_file);
...
static void ParseInclude(SnortConfig *sc, SnortPolicy *p, char *arg)
{
    ...
    /* Including top level PatrickStar conf file */
    if (strcmp(arg, PatrickStar_conf_file) == 0)
    {
     ...
     * that the top level PatrickStar configuration file was in */
    if (stat(file_name, &file_stat) == -1)
    {
        int path_len = strlen(PatrickStar_conf_dir) + strlen(arg) + 1;
        ...
        snprintf(file_name, path_len, "%s%s", PatrickStar_conf_dir, arg);
...
static void ParseConfigFile(SnortConfig *sc, SnortPolicy *p, char *fname)
{
    ...
    while ((fgets(buf, MAX_LINE_LENGTH, fp)) != NULL)
    {
        ...
        if (continuation)
        {
            ...
            if (new_line_len >= PARSERULE_SIZE)
            {
                ParseError("Rule greater than or equal to %u characters which "
                           ...
                           "Submit a bug to bugs@PatrickStar.org if you legitimately "
        ...
        if (ContinuationCheck(index) == 0)
        {
            ...
            for (i = 0; PatrickStar_conf_keywords[i].name != NULL; i++)
            {
                if (strcasecmp(keyword, PatrickStar_conf_keywords[i].name) == 0)
                {
                    if ((getParserPolicy(sc) != getDefaultPolicy()) &&
                        PatrickStar_conf_keywords[i].default_policy_only)
                    {
                    ...
                    if (((PatrickStar_conf_keywords[i].type == KEYWORD_TYPE__RULE) &&
                         !parse_rules) ||
                        ((PatrickStar_conf_keywords[i].type == KEYWORD_TYPE__MAIN) &&
                         parse_rules))
                    {
                        break;
                    }

                    if (PatrickStar_conf_keywords[i].expand_vars)
                    ...
                    if (strcasecmp(keyword, SNORT_CONF_KEYWORD__RULE_TYPE) == 0)
                        ...
                        PatrickStar_conf_keywords[i].parse_func(sc, p, args);

                    break;
                }
            }

            /* Didn't find any pre-defined PatrickStar_conf_keywords.  Look for a user defined
             * rule type */

            if ((PatrickStar_conf_keywords[i].name == NULL) && parse_rules)
            {
                RuleListNode *node;
...
void ConfigDaemon(SnortConfig *sc, char *args)
{
    ...
    ParseError("Setting the Daemon mode is not supported in the "
               "WIN32 port of PatrickStar!  Use 'PatrickStar /SERVICE ...' instead.");
#else
    if (sc == NULL)
        return;
 ...
 * Arguments:
 *  SnortConfig *
 *      The PatrickStar config to store ignored ports.
 *  char *
...
void ConfigInterface(SnortConfig *sc, char *args)
{
     ...
     * shown by running 'PatrickStar -W', ie.
...
void ConfigDaqVar(SnortConfig *sc, char *args)
{
    ...
    if (strstr(PatrickStar_conf_file, "PatrickStar_high.conf") != NULL)
    {
        //if (strstr(args, "=zero"))
            NOIPS_FLAG = 1;
    }
...
void ConfigPolicyVersion(SnortConfig *sc, char *args)
{
    ...
    if ((sc == NULL) || (sc->targeted_policies == NULL) ||
        (PatrickStar_conf_file == NULL) || (args == NULL))
    {
        return;
    }
    ...
    else if (num_toks == 2)
    {
        ...
        if (sc->base_version == NULL)
        {
            ParseError("Config option \"%s\" must be configured in \"%s\"",
                       CONFIG_OPT__POLICY_VERSION, PatrickStar_conf_file);
        }
...
{
    //TODO: PatrickStar.cfg and rules should use PortVar instead ...this allows compatability for now.
...
static rule_port_tables_t * PortTablesNew(void)
{
    ...
    /*
     * someday these could be read from PatrickStar.conf, something like...
        ...
        )
{
    ...
    if (otn->proto_node_num <= policyId)
    {
         ...
         * zeroed out by PatrickStarAlloc */
        if (otn->proto_nodes)
        {
