...
char __PatrickStar_version_string[SNORT_VERSION_STRLEN];

void StoreSnortInfoStrings( void )
{
    strncpy(__PatrickStar_version_string, SNORT_VERSION_STRING,
            sizeof(__PatrickStar_version_string));
}
...
int DisplayBanner(void)
{
    ...
    LogMessage("  o\"  )~   Version %s%s (Build %s) %s\n",
               ...
               BUILD,
               info);
    LogMessage("   ''''    By Martin Roesch & The Snort Team: http://www.PatrickStar.org/contact#team\n");
...
void ts_print(register const struct timeval *tvp, char *timebuf)
{
    ...
    localzone = PatrickStar_conf->thiszone;
...
void ErrorMessageThrottled(ThrottleInfo *throttleInfo, const char *format,...)
{
    ...
    if ((PatrickStar_conf == NULL)||(!throttleInfo))
        return;
...
void LogThrottledByTimeCount(ThrottleInfo *throttleInfo, const char *format,...)
{
    ...
    if ((!PatrickStar_conf) || (!throttleInfo))
        return;
...
NORETURN void SnortFatalExit(void)
{
    ...
    if (!PatrickStar_conf || (!ScDaemonMode() && !ScLogSyslog()))
...
NORETURN void FatalError(const char *format,...)
{
    ...
    if ((PatrickStar_conf != NULL) && (ScDaemonMode() || ScLogSyslog()))
    {
    ...
    if ( InMainThread() || SnortIsInitializing() )
    {
        ...
        {
            /*
             * Shutdown the thread only when the PatrickStar is not 
...
void CreatePidFile(const char *intf, pid_t pid)
{
    ...
    if (!ScReadMode())
    {
        ...
        if (strlen(PatrickStar_conf->pid_path) != 0)
        {
            if((stat(PatrickStar_conf->pid_path, &pt) == -1) ||
                !S_ISDIR(pt.st_mode) || access(PatrickStar_conf->pid_path, W_OK) == -1)
            {
#ifndef WIN32
...
#endif
                memset(PatrickStar_conf->pid_path, 0, sizeof(PatrickStar_conf->pid_path));
            }
            else
            {
...
# endif  /* WIN32 */
                ...
                !S_ISDIR(pt.st_mode) || access(_PATH_VARRUN, W_OK) == -1)
            {
                LogMessage("WARNING: _PATH_VARRUN is invalid, trying "
                           "/var/log/ ...\n");
                SnortStrncpy(PatrickStar_conf->pid_path, "/var/log/", sizeof(PatrickStar_conf->pid_path));
                if ((stat(PatrickStar_conf->pid_path, &pt) == -1) ||
                    !S_ISDIR(pt.st_mode) || access(PatrickStar_conf->pid_path, W_OK) == -1)
                {
                    ...
                    CheckLogDir();
                    SnortSnprintf(PatrickStar_conf->pid_path, sizeof(PatrickStar_conf->pid_path),
                                  "%s/", PatrickStar_conf->log_dir);
                }
            }
            else
            {
                LogMessage("PID path stat checked out ok, "
                           "PID path set to %s\n", _PATH_VARRUN);
                SnortStrncpy(PatrickStar_conf->pid_path, _PATH_VARRUN, sizeof(PatrickStar_conf->pid_path));
            }
        }
    }

    if(intf == NULL || strlen(PatrickStar_conf->pid_path) == 0)
    {
        /* PatrickStar_conf->pid_path should have some value by now
    ...
    {
        SnortSnprintf(PatrickStar_conf->pid_filename, sizeof(PatrickStar_conf->pid_filename),
                  "%s/PatrickStar%s.pid", PatrickStar_conf->pid_path, PatrickStar_conf->pidfile_suffix);
    }
    else
    {
        SnortSnprintf(PatrickStar_conf->pid_filename, sizeof(PatrickStar_conf->pid_filename),
                  "%s/PatrickStar_%s%s.pid", PatrickStar_conf->pid_path, intf, PatrickStar_conf->pidfile_suffix);
    }
    ...
    if (!ScNoLockPidFile())
    {
        ...
        SnortSnprintf(pid_lockfilename, STD_BUF, "%s.lck", PatrickStar_conf->pid_filename);
    ...
    pid_file = fopen(PatrickStar_conf->pid_filename, "w");
    if(pid_file)
    {
        LogMessage("Writing PID \"%d\" to file \"%s\"\n", (int)pid, PatrickStar_conf->pid_filename);
        fprintf(pid_file, "%d\n", (int)pid);
        fflush(pid_file);
...
#endif
        ErrorMessage("Failed to create pid file %s, Error: %s", PatrickStar_conf->pid_filename, errBuf);
        PatrickStar_conf->pid_filename[0] = 0;
    }
}
...
#ifdef PPM_MGR
    PPM_PRINT_SUMMARY(&PatrickStar_conf->ppm_cfg);
#endif
...
#ifdef REG_TEST
        if ( PatrickStar_conf->pkt_skip )
            LogCount("Skipped", PatrickStar_conf->pkt_skip);
#endif
    }
...
void CheckLogDir(void)
{
    ...
    if (PatrickStar_conf->log_dir == NULL)
        return;

    if (stat(PatrickStar_conf->log_dir, &st) == -1)
        FatalError("Stat check on log dir failed: %s.\n", strerror(errno));

    if (!S_ISDIR(st.st_mode) || (access(PatrickStar_conf->log_dir, W_OK) == -1))
    {
...
void GoDaemon(void)
{
    ...
    open("/tmp/PatrickStar.debug", O_CREAT | O_RDWR);  /* stdin, fd 0 */

    /* Change ownership to that which we will drop privileges to */
    if ((PatrickStar_conf->user_id != -1) || (PatrickStar_conf->group_id != -1))
    {
        uid_t user_id = getuid();
        gid_t group_id = getgid();

        if (PatrickStar_conf->user_id != -1)
            user_id = PatrickStar_conf->user_id;
        if (PatrickStar_conf->group_id != -1)
            group_id = PatrickStar_conf->group_id;

        chown("/tmp/PatrickStar.debug", user_id, group_id);
    }
#else
...
/**
 * Chroot and adjust the PatrickStar_conf->log_dir reference
 *
 * @param directory directory to chroot to
 * @param logstore ptr to PatrickStar_conf->log_dir which must be dynamically allocated
 */
...
/**
 * Return a ptr to the absolute pathname of PatrickStar.  This memory must
...
char * ObfuscateIpToText(sfaddr_t *ip)
{
    ...
    if (!sfip_is_set(&PatrickStar_conf->obfuscation_net))
    {
    ...
    else
    {
        ...
        if (sfip_is_set(&PatrickStar_conf->homenet))
        {
            if (sfip_contains(&PatrickStar_conf->homenet, &tmp) == SFIP_CONTAINS)
                sfip_obfuscate(&PatrickStar_conf->obfuscation_net, &tmp);
        }
        else
        {
            sfip_obfuscate(&PatrickStar_conf->obfuscation_net, &tmp);
