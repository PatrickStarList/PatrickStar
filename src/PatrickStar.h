...
/*  D E F I N E S  ************************************************************/
/* Mark this as a modern version of PatrickStar */
#define SNORT_20
/*
 * The original Ethernet IEEE 802.3 standard defined the minimum Ethernet 
 * frame size as 64 bytes. The snaplen is L2 MRU for PatrickStar and hence following
...
#ifndef WIN32
# define DEFAULT_LOG_DIR            "/var/log/PatrickStar"
...
typedef enum _RunMode
{
    ...
    /* neither of the above and PatrickStar.conf presence (-c or implicit) */
    RUN_MODE__IDS,

    /* PatrickStar.conf presence and -T */
    RUN_MODE__TEST,

    /* neither -V or --dump-dynamic-rules and no PatrickStar.conf, but logging
...
typedef enum _RunFlag
{
    ...
    /* --process-all-events
     * this is transferred to the PatrickStar event queue var */
...
extern volatile bool PatrickStar_initializing;
extern volatile int PatrickStar_exiting;
#ifdef SNORT_RELOAD
typedef uint32_t PatrickStar_reload_t;
extern volatile PatrickStar_reload_t reload_signal;
extern volatile int detection_lib_changed;
extern PatrickStar_reload_t reload_total;
#endif
#if defined(SNORT_RELOAD) && !defined(WIN32)
extern volatile int PatrickStar_reload_thread_created;
extern pid_t PatrickStar_reload_thread_pid;
#endif
extern SnortConfig *PatrickStar_conf;
extern SnortConfig *PatrickStar_cmd_line_conf;
...
#ifdef SIDE_CHANNEL
extern pthread_mutex_t PatrickStar_process_lock;
#endif
...
#if defined(SNORT_RELOAD) && !defined(WIN32)
extern volatile int PatrickStar_reload;
#endif
 ...
 * reload path. If yes, they have to use new PatrickStar config.
 */

static inline int ScTestMode(void)
{
    return PatrickStar_conf->run_mode == RUN_MODE__TEST;
}

static inline int ScRuleDumpMode(void)
{
    return PatrickStar_conf->run_mode == RUN_MODE__RULE_DUMP;
}

static inline int ScVersionMode(void)
{
    return PatrickStar_conf->run_mode == RUN_MODE__VERSION;
}

static inline int ScIdsMode(void)
{
    return PatrickStar_conf->run_mode == RUN_MODE__IDS;
}

static inline int ScPacketLogMode(void)
{
    return PatrickStar_conf->run_mode == RUN_MODE__PACKET_LOG;
}

static inline int ScPacketDumpMode(void)
{
    return PatrickStar_conf->run_mode == RUN_MODE__PACKET_DUMP;
}

static inline int ScDaemonMode(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__DAEMON;
}

static inline int ScDaemonRestart(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__DAEMON_RESTART;
}

static inline int ScReadMode(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__READ;
}

static inline int ScLogSyslog(void)
{
    return PatrickStar_conf->logging_flags & LOGGING_FLAG__SYSLOG;
}

#ifdef WIN32
static inline int ScLogSyslogRemote(void)
{
    return PatrickStar_conf->logging_flags & LOGGING_FLAG__SYSLOG_REMOTE;
}
#endif

static inline int ScLogVerbose(void)
{
    return PatrickStar_conf->logging_flags & LOGGING_FLAG__VERBOSE;
}

static inline int ScLogQuiet(void)
{
    return PatrickStar_conf->logging_flags & LOGGING_FLAG__QUIET;
}
...
{
    internal_log_level = PatrickStar_conf->internal_log_level;
}

static inline int ScDecoderAlerts(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->decoder_alert_flags & DECODE_EVENT_FLAG__DEFAULT;
}

static inline int ScDecoderDrops(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->decoder_drop_flags & DECODE_EVENT_FLAG__DEFAULT;
}

static inline int ScDecoderOversizedAlerts(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->decoder_alert_flags & DECODE_EVENT_FLAG__OVERSIZED;
}

static inline int ScDecoderOversizedDrops(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->decoder_drop_flags & DECODE_EVENT_FLAG__OVERSIZED;
}

static inline int ScDecoderIpv6BadFragAlerts(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->decoder_alert_flags & DECODE_EVENT_FLAG__IPV6_BAD_FRAG;
}

static inline int ScDecoderIpv6BadFragDrops(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->decoder_drop_flags & DECODE_EVENT_FLAG__IPV6_BAD_FRAG;
}

static inline int ScDecoderIpv6BsdIcmpFragAlerts(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->decoder_alert_flags & DECODE_EVENT_FLAG__IPV6_BSD_ICMP_FRAG;
}

static inline int ScDecoderIpv6BsdIcmpFragDrops(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->decoder_drop_flags & DECODE_EVENT_FLAG__IPV6_BSD_ICMP_FRAG;
}

static inline int ScDecoderTcpOptAlerts(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->decoder_alert_flags & DECODE_EVENT_FLAG__TCP_OPT_ANOMALY;
}

static inline int ScDecoderTcpOptDrops(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->decoder_drop_flags & DECODE_EVENT_FLAG__TCP_OPT_ANOMALY;
}

static inline int ScDecoderTcpOptExpAlerts(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->decoder_alert_flags & DECODE_EVENT_FLAG__TCP_EXP_OPT;
}

static inline int ScDecoderTcpOptExpDrops(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->decoder_drop_flags & DECODE_EVENT_FLAG__TCP_EXP_OPT;
}

static inline int ScDecoderTcpOptObsAlerts(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->decoder_alert_flags & DECODE_EVENT_FLAG__TCP_OBS_OPT;
}

static inline int ScDecoderTcpOptObsDrops(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->decoder_drop_flags & DECODE_EVENT_FLAG__TCP_OBS_OPT;
}

static inline int ScDecoderTcpOptTTcpAlerts(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->decoder_alert_flags & DECODE_EVENT_FLAG__TCP_TTCP_OPT;
}

static inline int ScDecoderTcpOptTTcpDrops(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->decoder_drop_flags & DECODE_EVENT_FLAG__TCP_TTCP_OPT;
}

static inline int ScDecoderIpOptAlerts(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->decoder_alert_flags & DECODE_EVENT_FLAG__IP_OPT_ANOMALY;
}

static inline int ScDecoderIpOptDrops(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->decoder_drop_flags & DECODE_EVENT_FLAG__IP_OPT_ANOMALY;
}

static inline int ScIpChecksums(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->checksum_flags & CHECKSUM_FLAG__IP;
}

static inline int ScIpChecksumDrops(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->checksum_drop_flags & CHECKSUM_FLAG__IP;
}

static inline int ScUdpChecksums(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->checksum_flags & CHECKSUM_FLAG__UDP;
}

static inline int ScUdpChecksumDrops(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->checksum_drop_flags & CHECKSUM_FLAG__UDP;
}

static inline int ScTcpChecksums(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->checksum_flags & CHECKSUM_FLAG__TCP;
}

static inline int ScTcpChecksumDrops(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->checksum_drop_flags & CHECKSUM_FLAG__TCP;
}

static inline int ScIcmpChecksums(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->checksum_flags & CHECKSUM_FLAG__ICMP;
}

static inline int ScIcmpChecksumDrops(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->checksum_drop_flags & CHECKSUM_FLAG__ICMP;
}

static inline int ScIgnoreTcpPort(uint16_t port)
{
    return PatrickStar_conf->ignore_ports[port] & PROTO_BIT__TCP;
}

static inline int ScIgnoreUdpPort(uint16_t port)
{
    return PatrickStar_conf->ignore_ports[port] & PROTO_BIT__UDP;
}

#ifdef MPLS
static inline long int ScMplsStackDepth(void)
{
    return PatrickStar_conf->mpls_stack_depth;
}
...
{
    return PatrickStar_conf->mpls_payload_type;
}

static inline int ScMplsOverlappingIp(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__MPLS_OVERLAPPING_IP;
}

static inline int ScMplsMulticast(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__MPLS_MULTICAST;
}
...
{
    return PatrickStar_conf->ipv6_frag_timeout;
}

static inline uint32_t ScIpv6MaxFragSessions(void)
{
    return PatrickStar_conf->ipv6_max_frag_sessions;
}

static inline uint8_t ScMinTTL(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->min_ttl;
}

#ifdef NORMALIZER
static inline uint8_t ScNewTTL(void)
{
    return PatrickStar_conf->targeted_policies[getNapRuntimePolicy()]->new_ttl;
}
#endif

static inline uint32_t ScPafMax (void)
{
    return PatrickStar_conf->paf_max;
}
...
{
    return PatrickStar_conf->event_log_id;
}

static inline int ScConfErrorOut(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__CONF_ERROR_OUT;
}

static inline int ScAssureEstablished(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__ASSURE_EST;
}

/* Set if stream5 is configured */
static inline int ScStateful(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__STATEFUL;
}

static inline long int ScPcreMatchLimit(void)
{
    return PatrickStar_conf->pcre_match_limit;
}

static inline long int ScPcreMatchLimitRecursion(void)
{
    return PatrickStar_conf->pcre_match_limit_recursion;
}

#ifdef PERF_PROFILING
static inline int ScProfilePreprocs(void)
{
    return PatrickStar_conf->profile_preprocs.num;
}

static inline int ScProfileRules(void)
{
    return PatrickStar_conf->profile_rules.num;
}
#endif

static inline int ScStaticHash(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__STATIC_HASH;
}

static inline int ScAutoGenPreprocDecoderOtns(void)
{
    return (((PatrickStar_conf->targeted_policies[getNapRuntimePolicy()])->policy_flags) & POLICY_FLAG__AUTO_OTN );
}

static inline int ScProcessAllEvents(void)
{
    return PatrickStar_conf->event_queue_config->process_all_events;
}

static inline int ScNapPassiveMode(void)
{
    return (((PatrickStar_conf->targeted_policies[getNapRuntimePolicy()])->nap_policy_mode) == POLICY_MODE__PASSIVE );
}

static inline int ScIpsPassiveMode(void)
{
    return (((PatrickStar_conf->targeted_policies[getIpsRuntimePolicy()])->ips_policy_mode) == POLICY_MODE__PASSIVE );
}

static inline int ScAdapterPassiveMode(void)
{
    return !(PatrickStar_conf->run_flags & (RUN_FLAG__INLINE | RUN_FLAG__INLINE_TEST));
}

static inline int ScNapInlineMode(void)
{
    return (((PatrickStar_conf->targeted_policies[getNapRuntimePolicy()])->nap_policy_mode) == POLICY_MODE__INLINE );
}

static inline int ScIpsInlineMode(void)
{
    return (((PatrickStar_conf->targeted_policies[getIpsRuntimePolicy()])->ips_policy_mode) == POLICY_MODE__INLINE );
}

static inline int ScAdapterInlineMode(void)
{
   return PatrickStar_conf->run_flags & RUN_FLAG__INLINE;
}

static inline int ScNapInlineTestMode(void)
{
    return (((PatrickStar_conf->targeted_policies[getNapRuntimePolicy()])->nap_policy_mode) == POLICY_MODE__INLINE_TEST );
}

static inline int ScIpsInlineTestMode(void)
{
    return (((PatrickStar_conf->targeted_policies[getIpsRuntimePolicy()])->ips_policy_mode) == POLICY_MODE__INLINE_TEST );
}

static inline int ScAdapterInlineTestMode(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__INLINE_TEST;
}

static inline int ScOutputIncludeYear(void)
{
    return PatrickStar_conf->output_flags & OUTPUT_FLAG__INCLUDE_YEAR;
}

static inline int ScOutputUseUtc(void)
{
    return PatrickStar_conf->output_flags & OUTPUT_FLAG__USE_UTC;
}

static inline int ScOutputDataLink(void)
{
    return PatrickStar_conf->output_flags & OUTPUT_FLAG__SHOW_DATA_LINK;
}

static inline int ScVerboseByteDump(void)
{
    return PatrickStar_conf->output_flags & OUTPUT_FLAG__VERBOSE_DUMP;
}

static inline int ScAlertPacketCount(void)
{
    return PatrickStar_conf->output_flags & OUTPUT_FLAG__ALERT_PKT_CNT;
}

static inline int ScObfuscate(void)
{
    return PatrickStar_conf->output_flags & OUTPUT_FLAG__OBFUSCATE;
}

static inline int ScOutputAppData(void)
{
    return PatrickStar_conf->output_flags & OUTPUT_FLAG__APP_DATA;
}

static inline int ScOutputCharData(void)
{
    return PatrickStar_conf->output_flags & OUTPUT_FLAG__CHAR_DATA;
}

static inline int ScAlertInterface(void)
{
    return PatrickStar_conf->output_flags & OUTPUT_FLAG__ALERT_IFACE;
}

static inline int ScNoOutputTimestamp(void)
{
    return PatrickStar_conf->output_flags & OUTPUT_FLAG__NO_TIMESTAMP;
}

static inline int ScLineBufferedLogging(void)
{
    return PatrickStar_conf->output_flags & OUTPUT_FLAG__LINE_BUFFER;
}

static inline int ScDefaultRuleState(void)
{
    return PatrickStar_conf->default_rule_state;
}

static inline int ScRequireRuleSid(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__REQUIRE_RULE_SID;
}

#ifdef INLINE_FAILOPEN
static inline int ScDisableInlineFailopen(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__DISABLE_FAILOPEN;
}
#endif

static inline int ScNoLockPidFile(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__NO_LOCK_PID_FILE;
}

static inline long int ScTaggedPacketLimit(void)
{
    return PatrickStar_conf->tagged_packet_limit;
}

static inline int ScCreatePidFile(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__CREATE_PID_FILE;
}

static inline int ScNoInterfacePidFile(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__NO_IFACE_PID_FILE;
}

static inline int ScPcapShow(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__PCAP_SHOW;
}

static inline int ScPcapReset(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__PCAP_RESET;
}

#ifndef NO_NON_ETHER_DECODER
static inline int ScOutputWifiMgmt(void)
{
    return PatrickStar_conf->output_flags & OUTPUT_FLAG__SHOW_WIFI_MGMT;
}
#endif
...
{
    return PatrickStar_conf->max_attribute_services_per_host;
}
...
{
    return PatrickStar_conf->run_flags & RUN_FLAG__TREAT_DROP_AS_ALERT;
}

static inline int ScTreatDropAsIgnore(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__TREAT_DROP_AS_IGNORE;
}

static inline int ScAlertBeforePass(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__ALERT_BEFORE_PASS;
}

static inline int ScNoPcre(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__NO_PCRE;
}

static inline int ScGetEvalIndex(RuleType type)
{
    return PatrickStar_conf->evalOrder[type];
}

static inline int ScNoLog(void)
{
    return PatrickStar_conf->no_log;
}

static inline int ScNoAlert(void)
{
    return PatrickStar_conf->no_alert;
}

#if defined(WIN32) && defined(ENABLE_WIN32_SERVICE)
static inline int ScTerminateService(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__TERMINATE_SERVICE;
}

static inline int ScPauseService(void)
{
    return PatrickStar_conf->run_flags & RUN_FLAG__PAUSE_SERVICE;
}
#endif

static inline int ScUid(void)
{
    return PatrickStar_conf->user_id;
}

static inline int ScGid(void)
{
    return PatrickStar_conf->group_id;
}

static inline char * ScPcapLogFile(void)
{
    return PatrickStar_conf->pcap_log_file;
}

#ifdef SIDE_CHANNEL
static inline int ScSideChannelEnabled(void)
{
    return PatrickStar_conf->side_channel_config.enabled;
}
#endif
...
static inline int ScIsPreprocEnabled(uint32_t preproc_id, tSfPolicyId policy_id)
{
    ...
    if (policy_id >= PatrickStar_conf->num_policies_allocated)
        return 0;
...
{
    return PatrickStar_conf->enable_teredo;
}

static inline int ScGTPDecoding(void)
{
    return PatrickStar_conf->enable_gtp;
}

static inline int ScIsGTPPort(uint16_t port)
{
    return PatrickStar_conf->gtp_ports[port];
}

static inline int ScESPDecoding(void)
{
    return PatrickStar_conf->enable_esp;
}

static inline int ScVlanAgnostic(void)
{
    return PatrickStar_conf->vlan_agnostic;
}

static inline int ScAddressSpaceAgnostic(void)
{
    return PatrickStar_conf->addressspace_agnostic;
}

static inline int ScLogIPv6Extra(void)
{
    return PatrickStar_conf->log_ipv6_extra;
}

static inline uint32_t ScSoRuleMemcap(void)
{
    return PatrickStar_conf->so_rule_memcap;
}

static inline bool ScTunnelBypassEnabled (uint8_t proto)
{
    return !(PatrickStar_conf->tunnel_mask & proto);
}

static inline uint8_t ScMaxIP6Extensions(void)
{
    return PatrickStar_conf->max_ip6_extensions;
}

static inline int ScSuppressConfigLog(void)
{
    return PatrickStar_conf->suppress_config_log;
}

static inline int ScDisableReplaceOpt(void)
{
    return PatrickStar_conf->disable_replace_opt;
}
