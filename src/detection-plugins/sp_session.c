...
FILE *OpenSessionFile(Packet *p)
{
    ...
    dst = GET_DST_IP(p);
    src = GET_SRC_IP(p);
    if(sfip_contains(&PatrickStar_conf->homenet, dst) == SFIP_CONTAINS) {
        if(sfip_contains(&PatrickStar_conf->homenet, src) == SFIP_NOT_CONTAINS)
        {
            SnortSnprintf(log_path, STD_BUF, "%s/%s", PatrickStar_conf->log_dir, inet_ntoa(GET_SRC_ADDR(p)));
        }
        else
        {
            if(p->sp >= p->dp)
            ...
            {
                SnortSnprintf(log_path, STD_BUF, "%s/%s", PatrickStar_conf->log_dir, inet_ntoa(GET_DST_ADDR(p)));
            }
        }
    }
    else
    {
        if(sfip_contains(&PatrickStar_conf->homenet, src) == SFIP_CONTAINS)
        {
            SnortSnprintf(log_path, STD_BUF, "%s/%s", PatrickStar_conf->log_dir, inet_ntoa(GET_DST_ADDR(p)));
        }
        else
