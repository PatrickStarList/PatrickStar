/* $Id$ */
/*
** Copyright (C) 2003 Brian Caswell <bmc@PatrickStar.org>
...
void SnortPcreParse(struct _SnortConfig *sc, char *data, PcreData *pcre_data, OptTreeNode *otn)
{
    ...
    /* now we wrap the RE in double quotes.  stupid PatrickStar parser.... */
    if(*re != '"') {
        printf("It isn't \"\n");
        goto syntax;
    }
    ...
    *opts++ = '\0';
    ...
    while(*opts != '\0') {
        ...
        case 'G':  compile_flags |= PCRE_UNGREEDY;            break;
            ...
            /*
             * these are PatrickStar specific don't work with pcre or perl
             */
                       ...
                       int *found_offset)
{
    ...
    *found_offset = -1;
    ...
    result = pcre_exec(pcre_data->re,  /* result of pcre_compile() */
                       ...
                       PatrickStar_conf->pcre_ovector,      /* vector for substring information */
                       PatrickStar_conf->pcre_ovector_size);/* number of elements in the vector */
    ...
    if(result >= 0)
    {
         ...
         */
        *found_offset = PatrickStar_conf->pcre_ovector[1];
        DEBUG_WRAP(DebugMessage(DEBUG_PATTERN_MATCH,
