 ...
 * Arguments:
 *
 * filename of the output log (default: PatrickStar.log)
 *
 * Effect:
 *
...
#define DEFAULT_FILE  "PatrickStar.log"
...
static void LogTcpdumpInit(struct _SnortConfig *sc, char *args)
{
    ...
    PatrickStar_conf->log_tcpdump = 1;
...
static void TcpdumpInitLogFile(LogTcpdumpData *data, int nostamps)
{
    ...
    if (nostamps)
    {
        if(data->filename[0] == '/')
            ...
            value = SnortSnprintf(data->logdir, STD_BUF, "%s/%s", PatrickStar_conf->log_dir,
                                  data->filename);
    }
    else
    {
        if(data->filename[0] == '/')
            ...
            value = SnortSnprintf(data->logdir, STD_BUF, "%s/%s.%u", PatrickStar_conf->log_dir,
