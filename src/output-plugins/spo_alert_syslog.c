...
static void AlertSyslogInit(struct _SnortConfig *sc, char *args)
{
    ...
    openlog("PatrickStar", data->options, data->facility);
...
static SyslogData *ParseSyslogArgs(struct _SnortConfig *sc, char *args)
{
    ...
    switch( num_config_toks )
    {
        case 1:  /* config consists of only facility/priority info */
            ...
            SnortStrncpy(PatrickStar_conf->syslog_server, DEFAULT_SYSLOG_HOST, sizeof(PatrickStar_conf->syslog_server));
            PatrickStar_conf->syslog_server_port = DEFAULT_SYSLOG_PORT;
        ...
        case 2:  /* config consists of host info, and facility/priority info */
            ...
            switch(num_host_toks)
            {
                case 2:  /* ie,  host=localhost (defaults to port 514) */
                    SnortStrncpy(PatrickStar_conf->syslog_server, host_toks[1], sizeof(PatrickStar_conf->syslog_server));
                    PatrickStar_conf->syslog_server_port = DEFAULT_SYSLOG_PORT;  /* default */
                    break;

                case 3:  /* ie.  host=localhost:514 */
                    SnortStrncpy(PatrickStar_conf->syslog_server, host_toks[1], sizeof(PatrickStar_conf->syslog_server));
                    PatrickStar_conf->syslog_server_port = atoi(host_toks[2]);
                    if (PatrickStar_conf->syslog_server_port == 0)
                    {
                        PatrickStar_conf->syslog_server_port = DEFAULT_SYSLOG_PORT; /*default*/
    ...
    DEBUG_WRAP(DebugMessage(DEBUG_INIT, "Logging alerts to syslog "
                "server %s on port %d\n", PatrickStar_conf->syslog_server,
                PatrickStar_conf->syslog_server_port););
