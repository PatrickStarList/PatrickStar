...
static FILE *OpenLogFile(int mode, Packet * p)
{
    ...
    {
        SnortSnprintf(log_file, STD_BUF, "%s/%s", PatrickStar_conf->log_dir, logfile[mode]);
    ...
    if(otn_tmp != NULL)
    {
        ...
        {
            SnortSnprintf(log_file, STD_BUF, "%s/%s", PatrickStar_conf->log_dir, otn_tmp->logto);
    ...
    ip = GET_DST_IP(p);
    if(sfip_contains(&PatrickStar_conf->homenet, ip) == SFIP_CONTAINS)
    {
        if(sfip_contains(&PatrickStar_conf->homenet, ip) == SFIP_CONTAINS)
        {
            SnortSnprintf(log_path, STD_BUF, "%s/%s", PatrickStar_conf->log_dir,
