...
{
    return Normalize_GetMode(PatrickStar_conf, nf) == NORM_MODE_OFF;
}
#else
    ...
    Packet *p, int sid, const char *str, int event_flag, int drop_flag)
{
    ...
    {
        queueDecoderEvent(GENERATOR_SNORT_DECODE, sid, 1, DECODE_CLASS, 3, str, 0);
...
static inline void PushLayer(PROTO_ID type, Packet* p, const uint8_t* hdr, uint32_t len)
{
    ...
    else
    {
        LogThrottledByTimeCount(&log_throttleInfo,
                "(PatrickStar_decoder) WARNING: Too many levels for decoding;"
                "next proto is %u.\n", type);
...
void DecodeARP(const uint8_t * pkt, uint32_t len, Packet * p)
{
    ...
    {
        DecoderEvent(p, DECODE_ARP_TRUNCATED, DECODE_ARP_TRUNCATED_STR, 1, 1);
        pc.discards++;
        return;
    }
...
void DecodeEthTypes(Packet *p, const uint8_t *pkt, uint16_t ethtype, uint32_t cap_len, uint8_t linklen)
{
    ...
    case ETHERNET_TYPE_IP:
        DecodeIP(pkt + linklen, cap_len - linklen, p);
        return;

    case ETHERNET_TYPE_ARP:
    case ETHERNET_TYPE_REVARP:
        DecodeARP(pkt + linklen, cap_len - linklen, p);
        return;

    case ETHERNET_TYPE_IPV6:
        DecodeIPV6(pkt + linklen, (cap_len - linklen), p);
        return;

    case ETHERNET_TYPE_PPPoE_DISC:
    case ETHERNET_TYPE_PPPoE_SESS:
        DecodePPPoEPkt(pkt + linklen, (cap_len - linklen), p);
        return;

#ifndef NO_NON_ETHER_DECODER
    case ETHERNET_TYPE_IPX:
        DecodeIPX(pkt + linklen, (cap_len - linklen), p);
        return;
#endif

    case ETHERNET_TYPE_LOOP:
        DecodeEthLoopback(pkt + linklen, (cap_len - linklen), p);
        return;
    ...
    case ETHERNET_TYPE_QINQ_NS2:
        DecodeVlan(pkt + linklen, cap_len - linklen, p);
        return;
#ifdef MPLS
    ...
    case ETHERNET_TYPE_MPLS_UNICAST:
        DecodeMPLS(pkt + linklen, cap_len - linklen, p);
        return;
#endif
    case ETHERNET_TYPE_CISCO_META:
        DecodeCiscoMeta(pkt + linklen, cap_len - linklen, p);
        return;
...
void DecodeEthPkt(Packet * p, const DAQ_PktHdr_t * pkthdr, const uint8_t * pkt)
{
    ...
    uint16_t proto = ETHERNET_TYPE_IP;
    PROFILE_VARS;
    ...
    /* do a little validation */
        ...
        {
            DEBUG_WRAP(DebugMessage(DEBUG_DECODE, "WARNING: Truncated eth header (%d bytes).\n", rem_len););
        ...
        if (p->eh->ether_type == 8)
            break;

        proto = ntohs(p->eh->ether_type);
        /* check if this is a FabricPath header */
        if(proto == ETHERNET_TYPE_FPATH)
        {
            /* do a little validation */
    ...
    DecodeEthTypes(p, p->pkt, proto, cap_len, linklen);
    PREPROC_PROFILE_END(decodePerfStats);
}
...
void DecodePppPktEncapsulated(const uint8_t* pkt, const uint32_t len, Packet* p)
{
    ...
    if(len < 2)
    {
       // ErrorMessage("Length not big enough for even a single header or a one byte payload\n");
        return;
    }
...
void DecodeIP(const uint8_t * pkt, const uint32_t len, Packet * p)
{
    ...
    if(IP_VER((IPHdr*)pkt) != 4)
    {
        if ((p->packet_flags & PKT_UNSURE_ENCAP) == 0)
            DecoderEvent(p, DECODE_NOT_IPV4_DGRAM, DECODE_NOT_IPV4_DGRAM_STR, 1, 1);
    ...
    if ( !NOIPS_FLAG &&  Event_Enabled(DECODE_IP4_DF_OFFSET) )
        if ( p->df && p->frag_offset )
            DecoderEvent(p, EVARGS(IP4_DF_OFFSET), 1, 1);

    if ( !NOIPS_FLAG &&  Event_Enabled(DECODE_IP4_LEN_OFFSET) )
    ...
    if(!NOIPS_FLAG &&  Event_Enabled(DECODE_BAD_FRAGBITS))
    {

        if( p->mf && p->df )
        {
            DecoderEvent(p, DECODE_BAD_FRAGBITS, DECODE_BAD_FRAGBITS_STR, 1, 1);
        }
    }
    ...
    {
	if (!NOIPS_FLAG)
		fpEvalIpProtoOnlyRules(PatrickStar_conf->ip_proto_only_lists, p);
...
void DecodeICMP(const uint8_t * pkt, const uint32_t len, Packet * p)
{
    ...
    {
        DEBUG_WRAP(DebugMessage(DEBUG_DECODE, "WARNING: Truncated ICMP4 header (%d bytes).\n", len););
...
void DecodeICMPEmbeddedIP(const uint8_t *pkt, const uint32_t len, Packet *p)
{
    ...
    if (len < IP_HEADER_LEN)
    {
        ...
        DecoderEvent(p, DECODE_ICMP_ORIG_IP_TRUNCATED, DECODE_ICMP_ORIG_IP_TRUNCATED_STR, 1, 1);
        p->orig_family = NO_IP;
        p->orig_iph = NULL;
        return;
    }
    ...
    if ((GET_ORIG_IPH_VER(p) != 4) && !IS_IP6(p))
    {
        ...
        DecoderEvent(p, DECODE_ICMP_ORIG_IP_VER_MISMATCH, DECODE_ICMP_ORIG_IP_VER_MISMATCH_STR, 1, 1);
        p->orig_family = NO_IP;
        p->orig_iph = NULL;
        return;
    }
    ...
    if (len < hlen)
    {
        ...
        DecoderEvent(p, DECODE_ICMP_ORIG_DGRAM_LT_ORIG_IP, DECODE_ICMP_ORIG_DGRAM_LT_ORIG_IP_STR, 1, 1);
        p->orig_family = NO_IP;
        p->orig_iph = NULL;
        return;
    }
    ...
    if (orig_frag_offset == 0)
    {
        ...
        if (ip_len < 8)
        {
            DecoderEvent(p, DECODE_ICMP_ORIG_PAYLOAD_LT_64, DECODE_ICMP_ORIG_PAYLOAD_LT_64_STR, 1, 1);
            return;
        }
        ...
        {
            DecoderEvent(p, DECODE_ICMP_ORIG_PAYLOAD_GT_576, DECODE_ICMP_ORIG_PAYLOAD_GT_576_STR, 1, 1);
        }
    }
    else
    {
...
void DecodeIPV6Options(int type, const uint8_t *pkt, uint32_t len, Packet *p)
{
    ...
    {
        DecoderEvent(p, DECODE_IPV6_TRUNCATED_EXT, DECODE_IPV6_TRUNCATED_EXT_STR, 1, 1);
        return;
    }

    if (p->ip6_extension_count >= ScMaxIP6Extensions())
    {
        DecoderEvent(p, DECODE_IP6_EXCESS_EXT_HDR, DECODE_IP6_EXCESS_EXT_HDR_STR, 1, 1);
        return;
    }
    ...
    switch (type)
    {
        ...
        case IPPROTO_DSTOPTS:
            if (exthdr->ip6e_nxt == IPPROTO_ROUTING)
                DecoderEvent(p, DECODE_IPV6_DSTOPTS_WITH_ROUTING, DECODE_IPV6_DSTOPTS_WITH_ROUTING_STR, 1, 1);
            hdrlen = sizeof(IP6Extension) + (exthdr->ip6e_len << 3);
        ...
        case IPPROTO_FRAGMENT:
            ...
            {
                DecoderEvent(p, DECODE_ZERO_LENGTH_FRAG, DECODE_ZERO_LENGTH_FRAG_STR, 1, 1);
                return;
            }
            else
            {
...
void DecodeIPV6(const uint8_t *pkt, uint32_t len, Packet *p)
{
    ...
    {
        if ((p->packet_flags & PKT_UNSURE_ENCAP) == 0)
            DecoderEvent(p, DECODE_IPV6_TRUNCATED, DECODE_IPV6_TRUNCATED_STR, 1, 1);
    ...
    {
        if ((p->packet_flags & PKT_UNSURE_ENCAP) == 0)
            DecoderEvent(p, DECODE_IPV6_IS_NOT, DECODE_IPV6_IS_NOT_STR, 1, 1);
        goto decodeipv6_fail;
    }
...
void DecodeUDP(const uint8_t * pkt, const uint32_t len, Packet * p)
{
    ...
    {
        UDPMiscTests(p);
        if (p->sp == TEREDO_PORT || p->dp == TEREDO_PORT || ScDeepTeredoInspection())
        {
            if ( !p->frag_flag )
    ...
    if (ScGTPDecoding() && (ScIsGTPPort(p->sp)||ScIsGTPPort(p->dp)))
    {
        if ( !p->frag_flag )
...
void DecodeTCP(const uint8_t * pkt, const uint32_t len, Packet * p)
{
    ...
    if(TCP_ISFLAGSET(p->tcph, (TH_SYN)))
    {
        ...
        if( p->tcph->th_flags == TH_SYN )
        {
            ...
            if(InternalEventIsEnabled(PatrickStar_conf->rate_filter_config,
...
void DecodeTRPkt(Packet * p, const DAQ_PktHdr_t * pkthdr, const uint8_t * pkt)
{
    ...
    if(p->trhllc->dsap != IPARP_SAP && p->trhllc->ssap != IPARP_SAP)
    {
        DEBUG_WRAP( DebugMessage(DEBUG_DECODE, "DSAP and SSAP arent set to SNAP\n"););
        p->trhllc = NULL;
        PREPROC_PROFILE_END(decodePerfStats);
        return;
    }
...
void DecodeLinuxSLLPkt(Packet * p, const DAQ_PktHdr_t * pkthdr, const uint8_t * pkt)
{
    ...
    {
        //ErrorMessage("Captured data length < SLL header length (your " "libpcap is broken?)! (%d bytes)\n", cap_len);
        PREPROC_PROFILE_END(decodePerfStats);
        return;
    }
    /* lay the ethernet structure over the packet data */
...
void DecodeOldPflog(Packet * p, const DAQ_PktHdr_t * pkthdr, const uint8_t * pkt)
{
    ...
    {
       // ErrorMessage("Captured data length < Pflog header length! " "(%d bytes)\n", cap_len);
        PREPROC_PROFILE_END(decodePerfStats);
        return;
    }
...
void DecodePflog(Packet * p, const DAQ_PktHdr_t * pkthdr, const uint8_t * pkt)
{
    ...
    {
      //  ErrorMessage("Captured data length < minimum Pflog length! " "(%d < %lu)\n", cap_len, (unsigned long)PFLOG2_HDRMIN);
        PREPROC_PROFILE_END(decodePerfStats);
        return;
    }
...
void DecodePppSerialPkt(Packet * p, const DAQ_PktHdr_t * pkthdr, const uint8_t * pkt)
{
    ...
    {
        //ErrorMessage("Captured data length < PPP header length" " (%d bytes)\n", cap_len);
        PREPROC_PROFILE_END(decodePerfStats);
        return;
    }
