...
{
    return PatrickStar_conf->event_queue[qIndex];
}
...
static int LogSnortEvents(void * event, void * user)
{
    ...
    SNORT_EVENTQ_USER *PatrickStar_user = (SNORT_EVENTQ_USER *) user;
    OptTreeNode *otn;
    ...
    if (otn)
    {
        ...
        if (rtn)
        {
            PatrickStar_user->rule_alert = otn->sigInfo.rule_flushing;
            LogSnortEvent((Packet *) PatrickStar_user->pkt, otn, rtn, en->msg);
        }
    }
