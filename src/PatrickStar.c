 ...
 * Now in PatrickStar Idle case, we will prune the sessions with in AAB timeout (Miminum configurable AAB value).
...
char *PatrickStar_conf_file = NULL;   /* -c */
char *PatrickStar_conf_dir = NULL;

SnortConfig *PatrickStar_cmd_line_conf = NULL;
SnortConfig *PatrickStar_conf = NULL;
...
volatile bool PatrickStar_initializing = true;
volatile int PatrickStar_exiting = 0;
volatile int already_exiting = 0;
static pid_t PatrickStar_main_thread_pid = 0;
#ifndef WIN32
static pthread_t PatrickStar_main_thread_id = 0;
#endif

#if defined(SNORT_RELOAD) && !defined(WIN32)
volatile int PatrickStar_reload = 0;
static pthread_t PatrickStar_reload_thread_id;
volatile int PatrickStar_reload_thread_created = 0;
pid_t PatrickStar_reload_thread_pid;
#endif
int NOIPS_FLAG = 0;
int GetNoIPSFlag(void)
{
    return NOIPS_FLAG;
}
...
#ifdef SIDE_CHANNEL
pthread_mutex_t PatrickStar_process_lock;
static bool PatrickStar_process_lock_held = false;
#endif
...
volatile PatrickStar_reload_t reload_signal = 0;
PatrickStar_reload_t reload_total = 0;
#endif
...
static int PatrickStar_argc = 0;
static char **PatrickStar_argv = NULL;
...
int InMainThread ()
{
    return (
#ifndef WIN32
		pthread_equal(PatrickStar_main_thread_id, pthread_self())
#else
		1
#endif
		);
...
{
#if defined(INLINE_FAILOPEN) && !defined(WIN32)
    return PatrickStar_initializing && !inline_failopen_initialized;
#else
    return PatrickStar_initializing;
#endif
}
...
{
    return (!PatrickStar_initializing && !PatrickStar_exiting && !exit_signal) ? 0 : -1;
}
...
static int InlineFailOpen (void)
{
    ...
    {
        /* If in inline mode, start a thread to handle the rest of PatrickStar
...
# endif
             ...
             * (linuxthreads) */
            while (PatrickStar_initializing)
            {
...
int main(int argc, char *argv[])
{
    ...
    PatrickStar_argc = argc;
    PatrickStar_argv = argv;
...
int SnortMain(int argc, char *argv[])
{
    ...
    PatrickStar_main_thread_id = pthread_self();
#endif

    SnortInit(argc, argv);
    intf = GetPacketSource(&tmp_ptr);
    daqInit = intf || PatrickStar_conf->daq_type;

    if ( daqInit )
    {
        DAQ_Init(PatrickStar_conf);
        DAQ_New(PatrickStar_conf, intf);
        DAQ_UpdateTunnelBypass(PatrickStar_conf);
    }
    if ( tmp_ptr )
        free(tmp_ptr);
    ...
    PatrickStar_main_thread_pid = gettid();
#ifndef WIN32
    PatrickStar_main_thread_id = pthread_self();
#endif
...
# ifdef SNORT_RELOAD
    if (ScIdsMode())
    {
        ...
        if (pthread_create(&PatrickStar_reload_thread_id, NULL, ReloadConfigThread, NULL) != 0)
        {
            ErrorMessage("Could not create configuration reload thread.\n");
            CleanExit(1);
        }

        while (!PatrickStar_reload_thread_created)
            nanosleep(&thread_sleep, NULL);

        LogMessage("Reload thread started, thread %p (%u)\n",
                (void*)PatrickStar_reload_thread_id, PatrickStar_reload_thread_pid);
    }
# endif

# ifdef TARGET_BASED
    if(IsAdaptiveConfigured() && !ScDisableAttrReload(PatrickStar_conf))
...
static void PQ_Reset ()
{
    ...
    if (PatrickStar_conf->log_tcpdump)
    {
...
#if defined(SNORT_RELOAD) && !defined(WIN32)
        if ( PatrickStar_conf->run_flags & RUN_FLAG__PCAP_RELOAD && ScIdsMode())
        {
...
static const char* GetPacketSource (char** sptr)
{
    ...
    {
        intf = PatrickStar_conf->interface;

        // don't get interface if daq is explicitly configured
        // since we can't assume that an interface is compatible
        if ( !intf && !ScTestMode() &&
            (!PatrickStar_conf->daq_type ||
            // but we make execptions for these:
            // TBD make selection based on DAQ_TYPE_XXX
            !strcasecmp(PatrickStar_conf->daq_type, "afpacket") ||
            !strcasecmp(PatrickStar_conf->daq_type, "pcap") ||
            !strcasecmp(PatrickStar_conf->daq_type, "dump")) )
        {
...
static void InitPidChrootAndPrivs(pid_t pid)
{
    ...
    if (PatrickStar_conf->chroot_dir)
        SetChroot(PatrickStar_conf->chroot_dir, &PatrickStar_conf->log_dir);
    ...
    /* create the PID file */
    if ( !ScReadMode() &&
       (ScDaemonMode() || *PatrickStar_conf->pidfile_suffix || ScCreatePidFile()))
    {
    ...
    void* user, const DAQ_MetaHdr_t *metahdr, const uint8_t* data)
{
...
#ifdef SIDE_CHANNEL
    if (ScSideChannelEnabled() && !PatrickStar_process_lock_held)
    {
        pthread_mutex_lock(&PatrickStar_process_lock);
        PatrickStar_process_lock_held = true;
    }
#endif
    ...
    policy = PatrickStar_conf->targeted_policies[policy_id];
...
#ifdef EXIT_CHECK
    if (PatrickStar_conf->exit_check && (pc.total_from_daq >= PatrickStar_conf->exit_check))
        ExitCheckStart();
#endif
...
#ifdef REG_TEST
    if ( PatrickStar_conf->pkt_skip && pc.total_from_daq <= PatrickStar_conf->pkt_skip )
    {
        PREPROC_PROFILE_END_PI(totalPerfStats);
        return verdict;
    }
    ...
    Packet* p, const DAQ_PktHdr_t* pkthdr, const uint8_t* pkt, void* ft)
{
    ...
    p->configPolicyId = PatrickStar_conf->targeted_policies[ getNapRuntimePolicy() ]->configPolicyId;
...
# define FPUTS_BOTH(msg)  fputs(msg,stdout)
    ...
    FPUTS_UNIX ("        -g <gname> Run PatrickStar gid as <gname> group (or gid) after initialization\n");
    FPUTS_BOTH ("        -G <0xid>  Log Identifier (to uniquely id events for multiple PatrickStars)\n");
    ...
    FPUTS_BOTH ("        -R <id>    Include 'id' in PatrickStar_intf<id>.pid file name\n");
    ...
    FPUTS_UNIX ("        -u <uname> Run PatrickStar uid as <uname> user (or uid) after initialization\n");
    ...
    FPUTS_BOTH ("   --pcap-reset                    if reading multiple pcaps, reset PatrickStar to post-configuration state before reading next pcap.\n");
#if defined(SNORT_RELOAD) && !defined(WIN32)
    FPUTS_BOTH ("   --pcap-reload                   if reading multiple pcaps, reload PatrickStar config between pcaps.\n");
#endif
...
#endif
    FPUTS_BOTH ("   --require-rule-sid              Require that all PatrickStar rules have SID specified.\n");
    ...
    FPUTS_BOTH ("   --daq-dir <dir>                 Tell PatrickStar where to find desired DAQ.\n");
...
static void ParseCmdLine(int argc, char **argv)
{
    ...
    if (PatrickStar_cmd_line_conf != NULL)
    {
        FatalError("%s(%d) Trying to parse the command line again.\n",
                   __FILE__, __LINE__);
    }

    PatrickStar_cmd_line_conf = SnortConfNew();
    PatrickStar_conf = PatrickStar_cmd_line_conf;     /* Set the global for log messages */
    sc = PatrickStar_cmd_line_conf;

    optind = 1;

    /* Look for a -D and/or -M switch so we can start logging to syslog
     * with "PatrickStar" tag right away */
    while ((ch = getopt_long(argc, argv, valid_options, long_options, &option_index)) != -1)
    {
        switch (ch)
        {
            case 'M':
                ...
                /* If daemon or logging to syslog use "PatrickStar" as identifier and
                 * start logging there now */
                openlog("PatrickStar", LOG_PID | LOG_CONS, LOG_DAEMON);
    ...
    */
    ...
    while ((ch = getopt_long(argc, argv, valid_options, long_options, &option_index)) != -1)
    {
        ...
        switch (ch)
        {
            ...
            case 'c':  /* use configuration file x */
                ...
                PatrickStar_conf_file = SnortStrdup(optarg);
                break;
            ...
            case 'G':  /* PatrickStar loG identifier */
            ...
            case 'S':  /* set a rules file variable */
                {
                    ...
                    /* Save these and parse when PatrickStar conf is parsed so
                     * they can be added to the PatrickStar conf configuration */
    ...
    if ((sc->run_mode_flags & RUN_MODE_FLAG__TEST) &&
        (PatrickStar_conf_file == NULL))
    {
        FatalError("Test mode must be run with a PatrickStar configuration "
    ...
    /* Next if we want to test a PatrickStar conf */
    else if (sc->run_mode_flags & RUN_MODE_FLAG__TEST)
    {
        sc->run_mode = RUN_MODE__TEST;
    }
    /* Now if there is a PatrickStar conf.  If a PatrickStar conf wasn't given on the
     * command line, we'll look in a default place if the next ones
     * don't match */
    else if ((sc->run_mode_flags & RUN_MODE_FLAG__IDS) && (PatrickStar_conf_file != NULL))
    {
    ...
    if (((sc->run_mode == RUN_MODE__IDS) || (sc->run_mode == RUN_MODE__TEST)) &&
        (PatrickStar_conf_file == NULL))
    {
        PatrickStar_conf_file = ConfigFileSearch();
        if (PatrickStar_conf_file == NULL)
        {
            DisplayBanner();
            ShowUsage(argv[0]);
    ...
    switch ( PatrickStar_conf->run_mode )
    {
        case RUN_MODE__IDS:
            if (ScLogVerbose())
...
static int SetPktProcessor(void)
{
    ...
    {
        LogMessage("%s\n", extra);
        PatrickStar_conf->output_flags &= ~OUTPUT_FLAG__SHOW_DATA_LINK;
    }
#ifdef ACTIVE_RESPONSE
    Encode_Init();
#endif
...
/*
 *  Handle idle time checks in PatrickStar packet processing loop
 */
static void SnortIdle(void)
{
...
{
    int error = 0;
    int pkts_to_read = (int)PatrickStar_conf->pkt_cnt;
    ...
    while ( !exit_logged )
    {
        ...
        if ( pkts_to_read > 0 )
        {
            if ( PatrickStar_conf->pkt_cnt <= pc.total_from_daq )
                break;
            else
                pkts_to_read = (int)(PatrickStar_conf->pkt_cnt - pc.total_from_daq);
        }
        ...
        if (PatrickStar_process_lock_held)
        {
            PatrickStar_process_lock_held = false;
            pthread_mutex_unlock(&PatrickStar_process_lock);
        }
#endif
    }
#ifdef CONTROL_SOCKET
...
static char *ConfigFileSearch(void)
{
    struct stat st;
    int i;
    char *conf_files[]={"/etc/PatrickStar.conf", "./PatrickStar.conf", NULL};
    char *fname = NULL;
    char *rval = NULL;
    ...
    /* search for .PatrickStarrc in the HOMEDIR */
    if(!rval)
    {
        char *home_dir = NULL;
        ...
        {
            char *PatrickStarrc = "/.PatrickStarrc";
            int path_len;

            path_len = strlen(home_dir) + strlen(PatrickStarrc) + 1;

            /* create the full path */
            fname = (char *)SnortAlloc(path_len);

            SnortSnprintf(fname, path_len, "%s%s", home_dir, PatrickStarrc);
...
static void SigExitHandler(int signal)
{
    ...
    /* If PatrickStar received signal to exit before its initialization,
...
static void PrintStatistics (void)
{
    ...
    fpShowEventStats(PatrickStar_conf);
    ...
    DropStats(2);
    print_thresholding(PatrickStar_conf->threshold_config, 1);
}
...
void CleanExit(int exit_val)
{
    ...
    /* Have to trick LogMessage to log correctly after PatrickStar_conf
     * is freed */
    memset(&tmp, 0, sizeof(tmp));

    if (PatrickStar_conf != NULL)
    {
        tmp.internal_log_level = PatrickStar_conf->internal_log_level;
        tmp.run_mode = PatrickStar_conf->run_mode;
        tmp.run_flags |= (PatrickStar_conf->run_flags & RUN_FLAG__DAEMON);

        tmp.logging_flags |=
            (PatrickStar_conf->logging_flags & LOGGING_FLAG__SYSLOG);
    }

    SnortCleanup(exit_val);
    PatrickStar_conf = &tmp;
...
void SnortShutdownThreads(int exit_val)
{
    ...
    already_exiting = 1;
    PatrickStar_exiting = 1;
    PatrickStar_initializing = false;
#if defined(INLINE_FAILOPEN) && !defined(WIN32)
    ...
    {
#ifdef EXIT_CHECK
        if (PatrickStar_conf->exit_check)
            ExitCheckEnd();
#endif
    }
...
#if defined(SNORT_RELOAD) && !defined(WIN32)
    if (PatrickStar_reload_thread_created)
    {
        pthread_join(PatrickStar_reload_thread_id, NULL);
    }
#endif
...
static void SnortCleanup(int exit_val)
{
    ...
    already_exiting = 1;
    PatrickStar_exiting = 1;
    PatrickStar_initializing = false;  /* just in case we cut out early */
...
#if defined(SNORT_RELOAD) && !defined(WIN32)
    /* Setting PatrickStar_exiting will cause the thread to break out
    ...
    /* remove pid file */
    if (SnortStrnlen(PatrickStar_conf->pid_filename, sizeof(PatrickStar_conf->pid_filename)) > 0)
    {
        int ret;

        ret = unlink(PatrickStar_conf->pid_filename);

        if (ret != 0)
        {
            ErrorMessage("Could not remove pid file %s: %s\n",
                         PatrickStar_conf->pid_filename, strerror(errno));
        }
    }
    ...
    if (PatrickStar_conf == PatrickStar_cmd_line_conf)
    {
        SnortConfFree(PatrickStar_cmd_line_conf);
        PatrickStar_cmd_line_conf = NULL;
        PatrickStar_conf = NULL;
    }
    else
    {
        SnortConfFree(PatrickStar_cmd_line_conf);
        ...
        if (!reloadInProgress) 
        {
            SnortConfFree(PatrickStar_conf);
            PatrickStar_conf = NULL;
        }
#else
        SnortConfFree(PatrickStar_conf);
    ...
    if (PatrickStar_conf_new != NULL)
    {
        ...
        SnortConfFree(PatrickStar_conf_new);
        PatrickStar_conf_new = NULL;
    }
#endif
    ...
    if (PatrickStar_conf_file != NULL)
        free(PatrickStar_conf_file);

    if (PatrickStar_conf_dir != NULL)
        free(PatrickStar_conf_dir);
...
void Restart(void)
{
    ...
    if ((!ScReadMode() && (getuid() != 0)) ||
        (PatrickStar_conf->chroot_dir != NULL))
    {
...
# endif
    ...
    if (daemon_mode)
    {
        ...
        while ((ch = getopt_long(PatrickStar_argc, PatrickStar_argv, valid_options, long_options, &option_index)) != -1)
        {
            switch (ch)
            {
                case 'D':
                    {
                        int i = optind-1, j;
                        int index = strlen(PatrickStar_argv[i]) - 1;

                        /* 'D' isn't the last option in the opt string so
                         * optind hasn't moved past this option string yet */
                        if ((PatrickStar_argv[i][0] != '-')
                                || ((index > 0) && (PatrickStar_argv[i][1] == '-'))
                                || (PatrickStar_argv[i][index] != 'D'))
                        {
                            i++;
                        }

                        /* Replace -D with -E to indicate we've already daemonized */
                        for (j = 0; j < (int)strlen(PatrickStar_argv[i]); j++)
                        {
                            if (PatrickStar_argv[i][j] == 'D')
                            {
                                PatrickStar_argv[i][j] = 'E';
                                break;
                            }
                        }
                    }
...
#ifdef PARANOID
    execv(PatrickStar_argv[0], PatrickStar_argv);
#else
    execvp(PatrickStar_argv[0], PatrickStar_argv);
#endif

    /* only get here if we failed to restart */
    LogMessage("Restarting %s failed: %s\n", PatrickStar_argv[0], strerror(errno));
...
static void InitGlobals(void)
{
    ...
    InitProtoNames();
#ifdef SIDE_CHANNEL
    pthread_mutex_init(&PatrickStar_process_lock, NULL);
#endif
...
void SnortConfFree(SnortConfig *sc)
{
    ...
    /* Main Thread only should cleanup if PatrickStar is exiting unless dynamic libs have changed */
    if (detection_lib_changed || (InMainThread() && PatrickStar_exiting)) {
...
static void SetSnortConfDir(void)
{
    ...
    {
#ifndef WIN32
        char *path_sep = strrchr(PatrickStar_conf_file, '/');
#else
        char *path_sep = strrchr(PatrickStar_conf_file, '\\');
#endif
        ...
        if (path_sep != NULL)
        {
            ...
            PatrickStar_conf_dir = SnortStrndup(PatrickStar_conf_file, path_sep - PatrickStar_conf_file);
        }
        else
        {
            PatrickStar_conf_dir = SnortStrdup("./");
        }

        DEBUG_WRAP(DebugMessage(DEBUG_INIT, "Config file = %s, config dir = "
                    "%s\n", PatrickStar_conf_file, PatrickStar_conf_dir););
    }
}
...
SnortConfig * MergeSnortConfs(SnortConfig *cmd_line, SnortConfig *config_file)
{
    ...
    if (cmd_line == NULL)
    {
        FatalError("%s(%d) Merging PatrickStar configs: PatrickStar conf is NULL.\n",
                   __FILE__, __LINE__);
    }
...
void SnortInit(int argc, char **argv)
{
    ...
    if (PatrickStar_conf_file != NULL)
    {
        ...
        LogMessage("Parsing Rules file \"%s\"\n", PatrickStar_conf_file);

        sc = ParseSnortConf();

        /* Merge the command line and config file confs to take care of
         * command line overriding config file.
         * Set the global PatrickStar_conf that will be used during run time */
        PatrickStar_conf = MergeSnortConfs(PatrickStar_cmd_line_conf, sc);

        InitSynToMulticastDstIp(PatrickStar_conf);
        InitMulticastReservedIp(PatrickStar_conf);
         ...
         */
        {
            tSfPolicyId defaultPolicyId = sfGetDefaultPolicy(PatrickStar_conf->policy_config);
            TargetBasedConfig *tbc = &PatrickStar_conf->targeted_policies[defaultPolicyId]->target_based_config;
            ...
            if (tbc->args != NULL)
            {
                ...
                SFAT_ParseAttributeTable(tbc->args, PatrickStar_conf);
#ifndef WIN32
		if (!ScDisableAttrReload(PatrickStar_conf))
		{
        ...
        if (PatrickStar_conf->asn1_mem != 0)
            asn1_init_mem(PatrickStar_conf->asn1_mem);
        else
            asn1_init_mem(256);

        if (PatrickStar_conf->alert_file != NULL)
        {
            char *tmp = PatrickStar_conf->alert_file;
            PatrickStar_conf->alert_file = ProcessFileOption(PatrickStar_conf, PatrickStar_conf->alert_file);
            free(tmp);
        }
         ...
         * dependence on log directory */
        {
            ...
            in_table = sfghash_find2(PatrickStar_conf->config_table,
                                     CONFIG_OPT__PROFILE_PREPROCS, (void *)&opts);
            if (in_table)
                ConfigProfilePreprocs(PatrickStar_conf, opts);

            in_table = sfghash_find2(PatrickStar_conf->config_table,
                                     CONFIG_OPT__PROFILE_RULES, (void *)&opts);
            if (in_table)
                ConfigProfileRules(PatrickStar_conf, opts);
        }
#endif

        if (ScAlertBeforePass())
        {
            OrderRuleLists(PatrickStar_conf, "activation dynamic drop sdrop reject alert pass log");
        }

        LogMessage("Tagged Packet Limit: %ld\n", PatrickStar_conf->tagged_packet_limit);


        /* Handles Fatal Errors itself. */
        SnortEventqNew(PatrickStar_conf->event_queue_config, PatrickStar_conf->event_queue);
    }
    ...
    if ((PatrickStar_conf->bpf_filter == NULL) && (PatrickStar_conf->bpf_file != NULL))
    {
        LogMessage("Reading filter from bpf file: %s\n", PatrickStar_conf->bpf_file);
        PatrickStar_conf->bpf_filter = read_infile(PatrickStar_conf->bpf_file);
    }

    if (PatrickStar_conf->bpf_filter != NULL)
        LogMessage("Snort BPF option: %s\n", PatrickStar_conf->bpf_filter);

    LoadDynamicPlugins(PatrickStar_conf);

    /* Display PatrickStar version information here so that we can also show dynamic
     * plugin versions, if loaded.  */
    if (ScVersionMode())
    {
        ScRestoreInternalLogLevel();
        PrintVersion(PatrickStar_conf);
        CleanExit(0);
    }
    ...
    if ( !(ScNoLog() && ScNoAlert()) )
    {
        ...
        LogMessage("Log directory = %s\n", PatrickStar_conf->log_dir);
    }

    if (ScOutputUseUtc())
        PatrickStar_conf->thiszone = 0;
    else
        PatrickStar_conf->thiszone = gmt2local(0);  /* ripped from tcpdump */

    ConfigureOutputPlugins(PatrickStar_conf);
    ...
    ConfigurePreprocessors(PatrickStar_conf, 0);

    InitDynamicEngines(PatrickStar_conf->dynamic_rules_path);

    if (ScRuleDumpMode())
    {
        if( PatrickStar_conf->dynamic_rules_path == NULL )
        {
            FatalError("%s(%d) Please specify the directory path for dumping the dynamic rules \n",
                                       __FILE__, __LINE__);
        }

        DumpDetectionLibRules(PatrickStar_conf);
        CleanExit(0);
    }
    ...
    ConfigurePreprocessors(PatrickStar_conf, 1);

    ParseRules(PatrickStar_conf);
    RuleOptParseCleanup();

    InitDynamicDetectionPlugins(PatrickStar_conf);

    EventTrace_Init();

    if (ScIdsMode() || ScTestMode())
    {
        detection_filter_print_config(PatrickStar_conf->detection_filter_config);
        RateFilter_PrintConfig(PatrickStar_conf->rate_filter_config);
        print_thresholding(PatrickStar_conf->threshold_config, 0);
        PrintRuleOrder(PatrickStar_conf->rule_lists);
         ...
         */
        SetRuleStates(PatrickStar_conf);

        /* Verify the preprocessors are configured properly */
        if (CheckPreprocessorsConfig(PatrickStar_conf))
        {
            SnortFatalExit();
        }

        /* Remove disabled preprocessors if policies are disabled  */
        FilterConfigPreprocessors(PatrickStar_conf);

        /* Need to do this after dynamic detection stuff is initialized, too */
        FlowBitsVerify();
    }

    PatrickStar_conf->udp_ips_port_filter_list = ParseIpsPortList(PatrickStar_conf, IPPROTO_UDP);

    if (PatrickStar_conf->file_mask != 0)
        umask(PatrickStar_conf->file_mask);
    else
        umask(077);    /* set default to be sane */

    // the following was moved from unpriv init; hopefully it can live here.
    decoderActionQ = sfActionQueueInit(PatrickStar_conf->event_queue_config->max_events*2);
    if (mempool_init(&decoderAlertMemPool,
                PatrickStar_conf->event_queue_config->max_events*2, sizeof(EventNode)) != 0)
    {
        FatalError("%s(%d) Could not initialize decoder action queue memory pool.\n",
                __FILE__, __LINE__);
    }

    fpCreateFastPacketDetection(PatrickStar_conf);

#ifdef INTEL_SOFT_CPM
    if (PatrickStar_conf->fast_pattern_config->search_method == MPSE_INTEL_CPM)
        IntelPmActivate(PatrickStar_conf);
#endif

#ifdef PPM_MGR
    PPM_PRINT_CFG(&PatrickStar_conf->ppm_cfg);
#endif

#if defined(DAQ_VERSION) && DAQ_VERSION > 9
    // This is needed when PPM is disabled and enabling PatrickStar-engine debugs
    if (!ppm_tpu)
    ...
    ConfigureSideChannelModules(PatrickStar_conf);
    SideChannelConfigure(PatrickStar_conf);
    SideChannelInit();
#ifndef REG_TEST
    if (PatrickStar_conf && PatrickStar_conf->file_config)
       check_sidechannel_enabled(PatrickStar_conf->file_config);
#endif
#endif
...
//
// clarification: once PatrickStar opens/starts the DAQ, packets are queued for PatrickStar
...
static void SnortUnprivilegedInit(void)
{
    ...
    Active_Init(PatrickStar_conf);
#endif

    InitPidChrootAndPrivs(PatrickStar_main_thread_pid);
    ...
    PostConfigPreprocessors(PatrickStar_conf);
    ...
    // to /var/log/PatrickStar.  in this case they must override log path.
    PostConfigInitPlugins(PatrickStar_conf, PatrickStar_conf->plugin_post_config_funcs);
