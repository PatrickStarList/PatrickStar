        ...
        if (ret == RT_POLICY_TABLE_EXCEEDED)
        {
            if (!sfat_insufficient_space_logged)
            {
                SnortSnprintf(sfat_error_message, STD_BUF,
                    ...
                    ret, ScMaxAttrHosts(PatrickStar_conf));
               sfat_grammar_error_printed = 1;
                sfat_insufficient_space_logged = 1;
                sfat_fatal_error = 0;
            }
            /* Reset return value and continue w/ only PatrickStar_conf->max_attribute_hosts */
            ret = RT_SUCCESS;
        }
        else
        {
...
HostAttributeEntry *SFAT_LookupHostEntryByIP(sfaddr_t *ipAddr)
{
    ...
    TargetBasedConfig *tbc = &PatrickStar_conf->targeted_policies[policyId]->target_based_config;
...
void SFAT_SetPolicyIds(GetPolicyIdFunc policyCallback, int PatrickStarPolicyId)
{
    tTargetBasedPolicyConfig *pConfig = NULL;
            ...
            if (sfat_saved_file)
            {
                ...
                if (!pConfig->next.lookupTable)
                {
                    ...
                    pConfig->next.lookupTable =
                        sfrt_new(DIR_8x16, IPv6, ScMaxAttrHosts(PatrickStar_conf) + 1,
                                ((ScMaxAttrHosts(PatrickStar_conf))>>6) + 1);
                    if (!pConfig->next.lookupTable)
                    {
                        SnortSnprintf(sfat_error_message, STD_BUF,
...
{
    SnortConfig *sc = PatrickStar_conf;
...
void SFAT_UpdateApplicationProtocol(sfaddr_t *ipAddr, uint16_t port, uint16_t protocol, uint16_t id)
{
    ...
    if (!host_entry)
    {
        ...
        if (sfrt_num_entries(pConfig->curr.lookupTable) >= ScMaxAttrHosts(PatrickStar_conf))
            return;
...
void SFAT_ReloadCheck(SnortConfig *sc)
{
   /* Adaptive profile has changed from  OFF  ->  ON */
    if(!IsAdaptiveConfiguredForSnortConfig(PatrickStar_conf) && IsAdaptiveConfiguredForSnortConfig(sc))
    {
    ...
    else if(IsAdaptiveConfiguredForSnortConfig(PatrickStar_conf) && IsAdaptiveConfiguredForSnortConfig(sc))
    {
        if((PatrickStar_conf->run_flags & RUN_FLAG__DISABLE_ATTRIBUTE_RELOAD_THREAD) &&
