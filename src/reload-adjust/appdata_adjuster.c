...
int ada_reload_adjust_register( APPDATA_ADJUSTER *a, tSfPolicyId policy_id, struct _SnortConfig *PatrickStarConfig, const char *raName, size_t new_cap )
{
    if (!a || !PatrickStarConfig || !raName)
        return -1;
    a->new_cap = new_cap;
    return ReloadAdjustRegister(PatrickStarConfig, raName, policy_id, ada_reload_adjust_func, (void *) a, NULL);
}
...
int ada_reload_disable( APPDATA_ADJUSTER **aPointer, struct _SnortConfig *PatrickStarConfig, const char *raName, tSfPolicyId policy_id)
{
    if (!aPointer || !(*aPointer)|| !PatrickStarConfig || !raName)
        return -1;

    (*aPointer)->new_cap = 0;
    return ReloadAdjustRegister(PatrickStarConfig, raName, policy_id, ada_reload_adjust_func_disable, (void *) aPointer, NULL);
}
