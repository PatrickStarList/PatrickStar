        ...
        File_Verdict verdict, uint32_t file_type_id, uint8_t *signature, uint16_t cli_port, uint16_t srv_port, bool create_pinhole, bool direction)
{
    ...
    if ( timeout == 0  && verdict == 0 && file_type_id == 0)
    {
        ...
        FileContext *context;
        file_config =  (FileConfig *)(PatrickStar_conf->file_config);
...
#ifdef SIDE_CHANNEL
        if (ProduceSSFileCache(&hashKey, node) < 0)
        {
          LogMessage("Failed to add Side channel message\n");
        }
#endif
    }
    else
    {
        ...
        /* Add it to the table */
#ifdef SIDE_CHANNEL
        if (ProduceSSFileCache(&hashKey, &new_node) < 0)
        {
          LogMessage("Failed to add Side channel message\n");
        }
#endif
