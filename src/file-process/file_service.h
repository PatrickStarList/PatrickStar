...
/* Initialize file API, this must be called when PatrickStar restarts */
void init_fileAPI(void);

/* Free file configuration, this must be called when PatrickStar reloads/restarts*/
void free_file_config(void*);

/* Swap the new config with current config */
void FileServiceInstall(void);

/* Close file API, this must be called when PatrickStar exits */
