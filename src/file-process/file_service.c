...
static int64_t get_max_file_depth(struct _SnortConfig *PatrickStar_conf, bool next);
...
{
   FileConfig *file_config =  (FileConfig *)(PatrickStar_conf->file_config);
...
static void init_file_context(void *ssnptr, bool upload, FileContext *context)
{
    ...
    context->file_config = PatrickStar_conf->file_config;
        ...
        FilePosition position)
{
    ...
    {
        if (get_max_file_depth(PatrickStar_conf, false) == (int64_t)context->processed_bytes)
            context->file_size = 0;
        else
        ...
        int data_size, FilePosition position, bool suspend_block_verdict)
{
    ...
    if ((context->file_config != PatrickStar_conf->file_config) ||
            (context->file_config_version != file_config_version))
    {
        context->file_config = PatrickStar_conf->file_config;
        context->file_config_version = file_config_version;
...
static int64_t get_max_file_capture_size(void *ssn)
{
    if (PatrickStar_conf->file_config)
        return PatrickStar_conf->file_config->file_capture_max_size;
