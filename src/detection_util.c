...
static void LogBuffer (const char* s, const uint8_t* p, unsigned n)
{
    ...
    if ( n > PatrickStar_conf->event_trace_max )
        n = PatrickStar_conf->event_trace_max;
...
{
    if ( PatrickStar_conf->event_trace_max > 0 )
    {
        time_t now = time(NULL);
        const char* ts = ctime(&now);

        char buf[STD_BUF];
        const char* dir = PatrickStar_conf->log_dir ? PatrickStar_conf->log_dir : ".";
        snprintf(buf, sizeof(buf), "%s/%s", dir, PatrickStar_conf->event_trace_file);

        tlog = TextLog_Init (buf, 128, 8*1024*1024);
        TextLog_Print(tlog, "\nTrace started at %s", ts);
        TextLog_Print(tlog, "Trace max_data is %u bytes\n", PatrickStar_conf->event_trace_max);
    }
}
