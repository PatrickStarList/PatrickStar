 ...
 *  standard PatrickStar search
 *  note: index is not used by PatrickStar, so it's commented
...
#ifdef BNFA_TRACK_Q
    if( bnfa->inq > PatrickStar_conf->max_inq )
        PatrickStar_conf->max_inq = bnfa->inq;
    PatrickStar_conf->tot_inq_flush += bnfa->inq_flush;
#endif
...
main (int argc, char **argv)
{
  ...
  //Write_DFA(acsm, "bnfa-PatrickStar.dfa") ;
