...
**        a) Full matrix
**        b) Sparse matrix
**        c) Banded matrix (Default-this is the only one used in PatrickStar)
...
{
#ifdef ACSMX2_TRACK_Q
    if( PatrickStar_conf->max_inq )
    {
        LogMessage("mpse: queue size     = %d, max possible = %d\n", PatrickStar_conf->max_inq, AC_MAX_INQ);
        LogMessage("mpse: queue flushes  = " STDu64 "\n", PatrickStar_conf->tot_inq_flush );
        LogMessage("mpse: queue inserts  = " STDu64 "\n", PatrickStar_conf->tot_inq_inserts );
        LogMessage("mpse: queue uinserts = " STDu64 "\n", PatrickStar_conf->tot_inq_uinserts );
    }
#endif
}
...
#ifdef ACSMX2_TRACK_Q
    if( q->inq > PatrickStar_conf->max_inq )
        PatrickStar_conf->max_inq = q->inq;
    PatrickStar_conf->tot_inq_flush += q->inq_flush;
#endif
 ...
 *   This feeds off of the last pattern groupd built within PatrickStar,
...
main (int argc, char **argv)
{
  ...
  Write_DFA(acsm, "acsmx2-PatrickStar.dfa") ;
