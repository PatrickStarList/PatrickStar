...
{
    return sc ? sc->parserPolicyId : PatrickStar_conf->parserPolicyId;
}
...
static inline void setParserPolicy(SnortConfig *sc, tSfPolicyId id)
{
    if (sc)
        ...
        PatrickStar_conf->parserPolicyId = id;
}

static inline int isParserPolicyDefault(SnortConfig *sc)
{
    return ( ( sc ? sc->parserPolicyId : PatrickStar_conf->parserPolicyId ) == SF_DEFAULT_POLICY_ID );
}
