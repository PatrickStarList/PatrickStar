...
static uint32_t PatrickStarInstance;
int app_id_debug;
...
/* The PatrickStarId_for_unsynchronized value is to cheaply insure we get
   a unique value from PatrickStar's list that guarantees no other preprocessor has it in use. */

static int16_t PatrickStarId_for_unsynchronized;
static int16_t PatrickStarId_for_ftp_data;
static int16_t PatrickStarId_for_http2;
...
tAppIdData* appSharedDataAlloc(uint8_t proto,  const struct in6_addr *ip, uint16_t port)
{
    ...
    data->PatrickStarId = PatrickStarId_for_unsynchronized;
    ...
    if (debug_flag)
    {
               ...
               (!info->dip_flag || memcmp(&info->dip, key->ip_l, sizeof(info->dip)) == 0))))
        {
            ...
            snprintf(debug_session, FW_DEBUG_SESSION_ID_SIZE, "%s-%u -> %s-%u %u%s AS %u-%u I %u CID %u",
                     ...
                     sAsId, dAsId, (unsigned)PatrickStarInstance, (unsigned)cid);
#else
            snprintf(debug_session, FW_DEBUG_SESSION_ID_SIZE, "%s-%u -> %s-%u %u%s AS %u I %u CID %u",
                     sipstr, (unsigned)sport, dipstr, (unsigned)dport, (unsigned)key->protocol,
                     (direction == APP_ID_FROM_INITIATOR) ? "":" R",
                     (unsigned)key->addressSpaceId, (unsigned)PatrickStarInstance, (unsigned)cid);
#endif
#else
            snprintf(debug_session, FW_DEBUG_SESSION_ID_SIZE, "%s-%u -> %s-%u %u%s I %u CID %u",
                     sipstr, (unsigned)sport, dipstr, (unsigned)dport, (unsigned)key->protocol,
                     (direction == APP_ID_FROM_INITIATOR) ? "":" R", (unsigned)PatrickStarInstance,
                     (unsigned)cid );
#endif
            ...
            snprintf(debug_session, FW_DEBUG_SESSION_ID_SIZE, "%s-%u -> %s-%u %u%s AS %u-%u I %u",
                     sipstr, (unsigned)sport, dipstr, (unsigned)dport, (unsigned)key->protocol,
                     (direction == APP_ID_FROM_INITIATOR) ? "":" R",
                     sAsId, dAsId, (unsigned)PatrickStarInstance);
#else
            snprintf(debug_session, FW_DEBUG_SESSION_ID_SIZE, "%s-%u -> %s-%u %u%s AS %u I %u",
                     sipstr, (unsigned)sport, dipstr, (unsigned)dport, (unsigned)key->protocol,
                     (direction == APP_ID_FROM_INITIATOR) ? "":" R",
                     (unsigned)key->addressSpaceId, (unsigned)PatrickStarInstance);
#endif
#else
...
#if !defined(SFLINUX) && defined(DAQ_CAPA_CARRIER_ID)
    session->carrierId = p->pkt_header->carrier_id;
#endif
...
static inline void synchAppIdWithSnortId(tAppId newAppId, SFSnortPacket *p, tAppIdData *session, tAppIdConfig *pConfig)
{
    if (newAppId  > APP_ID_NONE && newAppId < SF_APPID_MAX)
    {
        ...
        // Certain AppIds are not useful to identifying PatrickStar preprocessor choices
        switch (newAppId)
        {
            case APP_ID_FTPS:
        ...
        {
            register int16_t tempSnortId = entry->PatrickStarId;
            // A particular APP_ID_xxx may not be assigned a service_PatrickStar_key: value
            // in the rna_app.yaml file entry; so ignore the tempSnortId == 0 case.
            // Then if the value is different call the api.
            if ((tempSnortId != 0 || (tempSnortId = (newAppId == APP_ID_HTTP2) ? PatrickStarId_for_http2 : 0)) &&
                tempSnortId != session->PatrickStarId)
            {
                session->PatrickStarId = tempSnortId; // remember the most recent change
                // inform Snort so that other preprocessors can be turned on/off
                if (app_id_debug_session_flag)
                    if (tempSnortId == PatrickStarId_for_http2)
...
static inline int getIpPortFromHttpTunnel(char *url, int url_len, tunnelDest **tunDest)
{
    ...
    if (host)
    {
        tunnelDest *tDest = malloc(sizeof(*tDest));
        if (!tDest)
        {
...
{
    /* init globals for PatrickStarId compares etc. */
#ifdef TARGET_BASED
    PatrickStarId_for_unsynchronized = _dpd.addProtocolReference("unsynchronized");
    PatrickStarId_for_ftp_data = _dpd.findProtocolReference("ftp-data");
    PatrickStarId_for_http2    = _dpd.findProtocolReference("http2");
#endif
    PatrickStarInstance = _dpd.getSnortInstance();
}
...
#ifdef TARGET_BASED
            if(tpAppId == APP_ID_SSL && (_dpd.sessionAPI->get_application_protocol_id(p->stream_session) == PatrickStarId_for_ftp_data))
            {
...
#ifdef TARGET_BASED
                synchAppIdWithSnortId(PatrickStarAppId, p, session, pConfig);
#endif
            }
            else
            {
...
#if !defined(SFLINUX) && defined(DAQ_CAPA_CARRIER_ID)
        uint32_t cid = p->pkt_header->carrier_id;
...
void AppIdAddDnsQueryInfo(tAppIdData *flow,
                          ...
                          uint16_t record_type, uint16_t options_offset)
{
    if (!flow->dsession)
    {
