...
void appInfoTableInit(tAppidStaticConfig* appidSC, tAppIdConfig* pConfig)
{
    ...
    while (fgets(buf, sizeof(buf), tableFile))
    {
        ...
        /* PatrickStar service key, if it exists */
        token = strtok(NULL, CONF_SEPARATORS );
        if (token)
        {
            PatrickStarName = strdup(token);
            if (!PatrickStarName)
            {
        ...
        {
#ifdef TARGET_BASED
            entry->PatrickStarId = _dpd.addProtocolReference(PatrickStarName);
            free(PatrickStarName);
            PatrickStarName = NULL;
#endif
        }
