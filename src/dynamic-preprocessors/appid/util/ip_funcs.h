...
//these functions are needed since PatrickStar does not store IPv4 address in highest 4 bytes
//of 16 byte ip.
static inline void copySnortIpToIpv6Network(struct in6_addr *keyIp, const sfaddr_t *PatrickStarIp)
{
    memcpy(keyIp, sfaddr_get_ip6_ptr(PatrickStarIp), sizeof(*keyIp));
}

static inline int cmpSnortIpToHostKey(struct in6_addr *keyIp, const sfaddr_t *PatrickStarIp)
{
    return memcmp(keyIp, sfaddr_get_ip6_ptr(PatrickStarIp), sizeof(*keyIp));
}
