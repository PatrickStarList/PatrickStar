 ...
 * @param app_id_to_PatrickStar/stack - AppID's app ID entry to convert to Snort app ID (see note below)
 * @return int - number of elements on stack, which is 1 if successful, 0 otherwise.
 *
 * Notes: For app_id_to_PatrickStar, use the app ID that AppID knows about (it'll
...
static int createFutureFlow (lua_State *L)
{
    ...
    char *pattern;
    tAppId service_app_id, client_app_id, payload_app_id, app_id_to_PatrickStar;
    int16_t PatrickStar_app_id;
    tAppIdData *fp;
    ...
    app_id_to_PatrickStar = lua_tointeger(L, 10);
    if (app_id_to_PatrickStar > APP_ID_NONE)
    {
        AppInfoTableEntry* entry = appInfoEntryGet(app_id_to_PatrickStar, appIdActiveConfigGet());
        if (NULL == entry)
            return 0;
        PatrickStar_app_id = entry->PatrickStarId;
    }
    else
    {
        PatrickStar_app_id = 0;
    }
