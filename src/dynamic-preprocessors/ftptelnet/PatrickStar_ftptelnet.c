...
#include "pp_telnet.h"
#include "pp_ftp.h"
#include "PatrickStar_ftptelnet.h"
#include "sfPolicy.h"
 ...
 *
 * Any of the below can be overridden in PatrickStar.conf.
 *
 * This is here to eliminate most of it from PatrickStar.conf to
 * avoid an ugly configuration file.  The default_max_param_len
 * is somewhat arbitrary, but is taken from the majority of
 * the PatrickStar FTP rules that limit parameter size to 100
 ...
 *          with the highest priority, we then log that event through PatrickStar.
 *
...
int SnortFTPTelnet(SFSnortPacket *p)
{
     ...
     * the FTPTelnet library very independent from PatrickStar.
     */
