...
// global variables defined in PatrickStar_pop.c
        ...
        uint32_t len, uint64_t *flags, uint32_t* fp, uint32_t* fp_eoh)
{
    ...
    if (pfdata == NULL)
    {
        ...
        pfdata = _dpd.PatrickStarAlloc(1, sizeof(*pfdata), PP_POP, 0);
...
{
   if (pafData) {
      _dpd.PatrickStarFree(pafData, sizeof(PopPafData), PP_POP, 0);
   }
}
