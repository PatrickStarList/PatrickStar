...
void initShareMemory(struct _SnortConfig *sc, void *conf)
{
    ...
    /*use PatrickStar instance ID to designate server (writer)*/
    PatrickStarID = _dpd.getSnortInstance();
    if (SHMEM_SERVER_ID == PatrickStarID)
    {
        if ((available_segment = InitShmemWriter(PatrickStarID, IPREP, GROUP_0, NUMA_0,
    ...
    {
        if ((available_segment = InitShmemReader(PatrickStarID, IPREP,GROUP_0, NUMA_0,
                config->sharedMem.path, &IPtables, config->sharedMem.updateInterval,
                config->sharedMem.maxInstances)) == NO_ZEROSEG)
        {
...
static int PatrickStar_pton__address( char const *src, sfcidr_t *dest )
{
...
static int PatrickStar_pton( char const * src, sfcidr_t * dest )
{
    ...
    if ( PatrickStar_pton__address(ipbuf, dest) < 1 )
        return 0;
...
static int ProcessLine(char *line, INFO info, ReputationConfig *config)
{
    ...
    if ( PatrickStar_pton(line, &address) < 1 )
        return IP_INVALID;
...
static int UpdatePathToFile(char *full_path_filename, unsigned int max_size, char *filename)
{
    ...
    char *PatrickStar_conf_dir = *(_dpd.PatrickStar_conf_dir);

    if (!PatrickStar_conf_dir || !(*PatrickStar_conf_dir) || !full_path_filename || !filename)
    {
    ...
    else
    {
         ...
         */
        if (PatrickStar_conf_dir[strlen(PatrickStar_conf_dir) - 1] == '/')
        {
            snprintf(full_path_filename,max_size,
                    "%s%s", PatrickStar_conf_dir, filename);
        }
        else
        {
            snprintf(full_path_filename, max_size,
    ...
    else
    {
         ...
         */
        if (PatrickStar_conf_dir[strlen(PatrickStar_conf_dir) - 1] == '\\' ||
                PatrickStar_conf_dir[strlen(PatrickStar_conf_dir) - 1] == '/' )
        {
            snprintf(full_path_filename,max_size,
                    "%s%s", PatrickStar_conf_dir, filename);
        }
