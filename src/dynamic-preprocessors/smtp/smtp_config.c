...
void SMTP_ParseArgs(SMTPConfig *config, char *args)
{
    ...
    config->cmd_config = (SMTPCmdConfig *)_dpd.PatrickStarAlloc(CMD_LAST, sizeof(SMTPCmdConfig), PP_SMTP, 1);
...
static int AddCmd(SMTPConfig *config, char *name, SMTPCmdTypeEnum type)
{
    ...
    cmds = (SMTPToken *)_dpd.PatrickStarAlloc(config->num_cmds + 1, sizeof(SMTPToken), PP_SMTP, 1);
    if (cmds == NULL)
    {
        DynamicPreprocessorFatalMessage("%s(%d) => Failed to allocate memory for SMTP "
                                        "command structure\n",
    ...
    cmd_search = (SMTPSearch *)_dpd.PatrickStarAlloc(config->num_cmds, sizeof(SMTPSearch), PP_SMTP, 1);
    if (cmd_search == NULL)
    {
        DynamicPreprocessorFatalMessage("%s(%d) => Failed to allocate memory for SMTP "
                                        "command structure\n",
    ...
    cmd_config = (SMTPCmdConfig *)_dpd.PatrickStarAlloc(config->num_cmds, sizeof(SMTPCmdConfig), PP_SMTP, 1);
    if (cmd_config == NULL)
    {
        DynamicPreprocessorFatalMessage("%s(%d) => Failed to allocate memory for SMTP "
                                        "command structure\n",
