...
static void UpdatePathToDir(char *full_path_dirname, unsigned int max_size, char *dirname)
{
    ...
    if (!PatrickStar_conf_dir || !(*PatrickStar_conf_dir) || !full_path_dirname || !dirname)
    {
    ...
    else
    {
        ...
        if (PatrickStar_conf_dir[strlen(PatrickStar_conf_dir) - 1] == '/')
        {
            iRet = snprintf(full_path_dirname,max_size,
                    "%s%s", PatrickStar_conf_dir, dirname);
        }
        else
        {
            iRet = snprintf(full_path_dirname, max_size,
...
int SSLReloadVerify(struct _SnortConfig *sc, void *swap_config)
{
    ...
    {
        /* Reload mem adjust not available, restart PatrickStar. */
