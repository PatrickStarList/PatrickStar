 ...
 *
 * Arguments: PatrickStar.conf argument line for the DCE/RPC preprocessor.
 *
 * Returns: None
 *
 *********************************************************************/
 ...
 *
 * Arguments:   idle - if PatrickStar is idling (low packets) do more work
...
static void DCE2_AddPortsToPaf(struct _SnortConfig *sc, DCE2_Config *config, tSfPolicyId policy_id)
{
    ...
    if (config->sconfigs != NULL)
        sfrt_iterate_with_PatrickStar_config(sc, config->sconfigs, DCE2_ScAddPortsToPaf);

    dce2_paf_tmp_policy_id = 0;
}

static void DCE2_ScAddPortsToPaf(struct _SnortConfig *PatrickStarConf, void *data)
{
    ...
    for (port = 0; port < DCE2_PORTS__MAX; port++)
    {
        ...
        {
            DCE2_PafRegisterPort(PatrickStarConf, (uint16_t)port, policy_id, DCE2_TRANS_TYPE__SMB);
        }
        ...
        {
            DCE2_PafRegisterPort(PatrickStarConf, (uint16_t)port, policy_id, DCE2_TRANS_TYPE__TCP);
        }
