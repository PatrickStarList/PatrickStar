 ...
 *
 ****************************************************************************
 * Parses and processes configuration set in PatrickStar.conf.
 *
 ...
 * Arguments:
 *  char *
 *      PatrickStar.conf argument line for the dcerpc2 preprocessor.
 *
 * Returns: None
 *
 ********************************************************************/
 ...
 *
 ********************************************************************/
void DCE2_ServerConfigure(struct _SnortConfig *PatrickStarConf, DCE2_Config *config, char *args)
{
    DCE2_ServerConfig *sc;
    DCE2_Queue *ip_queue;
    DCE2_Ret status;
    tSfPolicyId policy_id = _dpd.getParserPolicy(PatrickStarConf);
    ...
    DCE2_AddPortsToStreamFilter(PatrickStarConf, sc, policy_id);
    DCE2_RegisterPortsWithSession(PatrickStarConf, sc);
 ...
 *
 *********************************************************************/
static void DCE2_AddPortsToStreamFilter(struct _SnortConfig *PatrickStarConf, DCE2_ServerConfig *sc, tSfPolicyId policy_id)
{
    unsigned int port;
    ...
    for (port = 0; port < DCE2_PORTS__MAX; port++)
    {
        if (DCE2_IsPortSet(sc->smb_ports, (uint16_t)port))
        {
            _dpd.streamAPI->set_port_filter_status
                (PatrickStarConf, IPPROTO_TCP, (uint16_t)port, PORT_MONITOR_SESSION, policy_id, 1);
        }
        ...
        {
            _dpd.streamAPI->set_port_filter_status
                (PatrickStarConf, IPPROTO_UDP, (uint16_t)port, PORT_MONITOR_SESSION, policy_id, 1);
        }
