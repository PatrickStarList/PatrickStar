...
**              Andrew R. Baker <andrewb@PatrickStar.org>
...
**  configuration issues that are set in PatrickStar.conf through
**  variables.
*/
...
**
**  DESCRIPTION
**    This function takes the corresponding RTN and OTN for a PatrickStar rule
...
**
**  FORMAL INPUTS
**    RuleTreeNode * - rtn for PatrickStar rule
**    OptTreeNode  * - otn for PatrickStar rule
...
int fpAddMatch(OTNX_MATCH_DATA *omd_local, int pLen, OptTreeNode *otn)
{
    ...
    */
    if( pmi->iMatchCount >= (int)PatrickStar_conf->fast_pattern_config->max_queue_events ||
...
static inline int fpFinalSelectEvent(OTNX_MATCH_DATA *o, Packet *p)
{
    ...
    OptTreeNode *otn;
    int tcnt = 0;
    EventQueueConfig *eq = PatrickStar_conf->event_queue_config;
    RuleTreeNode *rtn;
        ...
        int check_ports, char ip_rule, OTNX_MATCH_DATA *omd)
{
    ...
    char repeat = 0;
    FastPatternConfig *fp = PatrickStar_conf->fast_pattern_config;
    PROFILE_VARS;
    ...
    if (do_detect_content)
    {
         ...
         */
        {
            ...
            SnortPolicy *policy = PatrickStar_conf->targeted_policies[policy_id];
...
static inline int fpEvalHeaderUdp(Packet *p, OTNX_MATCH_DATA *omd)
{
    ...
    if (IsAdaptiveConfigured())
    {
        ...
        {
            prmFindGenericRuleGroup(PatrickStar_conf->prmUdpRTNX, &gen);

            /* TODO:  To From Server ?, else we apply  */
            dst = fpGetServicePortGroupByOrdinal(PatrickStar_conf->sopgTable, IPPROTO_UDP, TO_SERVER, proto_ordinal);
            src = fpGetServicePortGroupByOrdinal(PatrickStar_conf->sopgTable, IPPROTO_UDP, TO_CLIENT, proto_ordinal);
        }

        // Find non-service based rule groups and the generic rule groups
        prmFindNoServiceRuleGroup(PatrickStar_conf->prmUdpRTNX, p->dp, p->sp, &nssrc, &nsdst, &gen);
    }

    if ( src == NULL && dst == NULL )
    {
        if(!prmFindRuleGroupUdp(PatrickStar_conf->prmUdpRTNX, p->dp, p->sp, &src, &dst, &nssrc, &nsdst, &gen))
            return 0;
    }
    else
    {
...
#else
    if (!prmFindRuleGroupUdp(PatrickStar_conf->prmUdpRTNX, p->dp, p->sp, &src, &dst, &gen))
        return 0;
#endif

    if (fpDetectGetDebugPrintNcRules(PatrickStar_conf->fast_pattern_config))
    {
        LogMessage(
...
static inline int fpEvalHeaderTcp(Packet *p, OTNX_MATCH_DATA *omd)
{
    ...
    if (IsAdaptiveConfigured())
    {
        ...
        if (proto_ordinal > 0 && proto_ordinal != SFTARGET_UNKNOWN_PROTOCOL)
        {
            prmFindGenericRuleGroup(PatrickStar_conf->prmTcpRTNX, &gen);

            if (p->packet_flags & PKT_FROM_SERVER)
                src = fpGetServicePortGroupByOrdinal(PatrickStar_conf->sopgTable, IPPROTO_TCP, TO_CLIENT, proto_ordinal);

            if (p->packet_flags & PKT_FROM_CLIENT)
                dst = fpGetServicePortGroupByOrdinal(PatrickStar_conf->sopgTable, IPPROTO_TCP, TO_SERVER, proto_ordinal);
        }
    }

    if ( src == NULL && dst == NULL )
    {
        if(!prmFindRuleGroupTcp(PatrickStar_conf->prmTcpRTNX, p->dp, p->sp, &src, &dst, &nssrc, &nsdst, &gen))
            return 0;
    }
    else
    {
        prmFindNoServiceRuleGroup(PatrickStar_conf->prmTcpRTNX, p->dp, p->sp, &nssrc, &nsdst, &gen);
    }
#else
    if (!prmFindRuleGroupTcp(PatrickStar_conf->prmTcpRTNX, p->dp, p->sp, &src, &dst, &gen))
        return 0;
#endif
...
static inline int fpEvalHeaderIcmp(Packet *p, OTNX_MATCH_DATA *omd)
{
    ...
    if (!prmFindRuleGroupIcmp(PatrickStar_conf->prmIcmpRTNX, p->icmph->type, &type, &gen))
        return 0;
...
static inline int fpEvalHeaderIp(Packet *p, int ip_proto, OTNX_MATCH_DATA *omd)
{
    ...
    if (!prmFindRuleGroupIp(PatrickStar_conf->prmIpRTNX, ip_proto, &ip_group, &gen))
        return 0;
...
**    we are using a setwise methodology for PatrickStar rules, both the
...
int fpEvalPacket(Packet *p)
{
    ...
    OTNX_MATCH_DATA *omd = PatrickStar_conf->omd;
    ...
    )
{
    ...
    if (otn)
    {
        ...
        {
            OtnLookupAdd(PatrickStar_conf->otn_map, otn);
        }
        else
        {
            free(otn);
...
{
    OptTreeNode *otn = OtnLookup(PatrickStar_conf->otn_map, gid, sid);
