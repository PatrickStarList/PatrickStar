...
{
    ReloadAdjustEntry* rae = PatrickStar_conf->raEntry;
    ...
    if (rae)
    {
        ...
        if (rae != PatrickStar_conf->raCurrentEntry)
        {
            PatrickStar_conf->raCurrentEntry = rae;
            PatrickStar_conf->raLastLog = tv_sec;
        ...
        if (rae->raFunc(idle, rae->raPolicyId, rae->raUserData))
        {
            ...
            memset(&adjust_started_at, 0 , sizeof(adjust_started_at));
            PatrickStar_conf->raEntry = rae->raNext;
            free(rae);
        }
        else if (!PatrickStar_conf->raLastLog)
            PatrickStar_conf->raLastLog = tv_sec;
        else if ((unsigned)(tv_sec - PatrickStar_conf->raLastLog) >= 10)
        {
            PatrickStar_conf->raLastLog = tv_sec;
...
extern SnortConfig *PatrickStar_conf_new;
