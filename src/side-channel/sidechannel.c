...
int SideChannelEnqueueMessageRX(SCMsgHdr *hdr, const uint8_t *msg, uint32_t length, void *msg_handle, SCMQMsgFreeFunc msgFreeFunc)
{
    ...
    /*
     * During PatrickStar exit, any messages need not to be processed. So checking
     ...
     * This should be safe since the main thread holds the PatrickStar_process_lock mutex while it
     * is not in DAQ_Acquire().
     */
    while (pthread_mutex_trylock(&PatrickStar_process_lock) == 0)
    {
