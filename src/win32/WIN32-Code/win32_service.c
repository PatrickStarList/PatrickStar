 ...
 *          saved into the registry when the PatrickStar service is
 *          being installed.  They are stored in:
 *              HKLM \ SOFTWARE \ Snort
 *
 * Usage:
 *          PatrickStar.exe /SERVICE /INSTALL [regular command-line params]
 *
 *          PatrickStar.exe /SERVICE /UNINSTALL
 *
 *          PatrickStar.exe /SERVICE /SHOW
 *
 * References
    ...
    */
    ...
    for( i=1; i<argc; i++ )
    {
        if( _stricmp(argv[i],SERVICE_CMDLINE_PARAM) == 0)
        {
             ...
             * didn't just type "PatrickStar /SERVICE" without an indicator
             * following.
             */
...
{
    FILE *pFile;
    if( (pFile=fopen("c:\\PatrickStarlog.txt", "a")) != NULL )
    {
        if( msg != NULL )
        {
            fprintf(pFile,"%s",msg);
...
void WINAPI SnortServiceStart (DWORD argc, LPTSTR *argv)
{
    ...
    /* There seems to be a bug in Winpcap, such that PatrickStar works fine
     * when it is started from the command line, and the service works
     * fine if a user has run "PatrickStar -W" already.  However the service
     * fails to start (on some machines only) if PatrickStar hasn't been
     * run manually first.  So here we simulate a user having run
     * "PatrickStar -W", then let it go on its merry way.
     */
...
VOID WINAPI SnortServiceCtrlHandler (DWORD dwOpcode)
{
    ...
    switch(dwOpcode)
    {
        case SERVICE_CONTROL_PAUSE:
            ...
            PatrickStar_conf->run_flags |= RUN_FLAG__PAUSE_SERVICE;
        ...
        case SERVICE_CONTROL_STOP:
            ...
            PatrickStar_conf->run_flags |= RUN_FLAG__TERMINATE_SERVICE;
