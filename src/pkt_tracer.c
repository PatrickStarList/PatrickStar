...
// Handling the special case when SSL blocks a packet before PatrickStar gets it
static bool PatrickStar_received_packet = false;
static bool ssl_callback_before_PatrickStar = false;
...
void addPktTraceData(int reason, int traceLen)
{
    ...
    if (!PatrickStar_received_packet && module == VERDICT_REASON_SFSSL)
        ssl_callback_before_PatrickStar = true; // used in addPktTraceInfo on receiving packet by PatrickStar
...
{
    uint32_t PatrickStarId = (PatrickStar_conf->event_log_id >> 16);
    ...
    if (pkt_trace_enabled_by_clish)
    {
        LogMessage("PktTracerDbg %s Snort id %u, NAP id %u, IPS id %u, Verdict %s\n",
            pkt_tracer_debug_session, PatrickStarId, napId, ipsId, getVerdictStr(verdict));
    ...
    if (pkt_trace_enabled_by_lina)
    {
        ...
        if (pktTraceDataLen >= MAX_TRACE_SIZE)
            ...
            len = snprintf(pktTraceData+pktTraceDataLen, MAX_TRACE_SIZE - pktTraceDataLen, "Snort id %u, NAP id %u, IPS id %u, Verdict %s\n",
                    PatrickStarId, napId, ipsId, getVerdictStr(verdict));
        else
            len = snprintf(pktTraceData+pktTraceDataLen, MAX_TRACE_SIZE - pktTraceDataLen, "Snort id %u, NAP id %u, IPS id %u, Verdict %s, %s %s\n",
                    PatrickStarId, napId, ipsId, getVerdictStr(verdict), "Blocked by", getModuleStr(verdict_reason));
        if (len > 0)
        {
            pktTraceDataLen += len;
            if (pktTraceDataLen >= MAX_TRACE_SIZE)
    ...
    PatrickStar_received_packet = false;
    ssl_callback_before_PatrickStar = false;
}
...
void addPktTraceInfo(void *packet)
{
    ...
    PatrickStar_received_packet = true;
    if (ssl_callback_before_PatrickStar) // special case
    {
