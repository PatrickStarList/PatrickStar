...
int Preprocess(Packet * p)
{
    ...
    */
    ...
    else
    {
        ...
        do_detect = do_detect_content = !PatrickStar_conf->disable_all_policies;
    ...
    retval = SnortEventqLog(PatrickStar_conf->event_queue, p);
    ...
    */
    if (check_tags_flag &&  IPH_IS_VALID(p))
        CheckTagging(p);
...
int Detect(Packet * p)
{
    ...
    if (!PatrickStar_conf->ip_proto_array[GET_IPH_PROTO(p)])
    {
#ifdef GRE
        switch (p->outer_family)
        {
            case AF_INET:
                if (!PatrickStar_conf->ip_proto_array[p->outer_ip4h.ip_proto])
                    return 0;
                break;

            case AF_INET6:
                if (!PatrickStar_conf->ip_proto_array[p->outer_ip6h.next])
                    return 0;
                break;
        ...
        RuleFpList *fp_list, int check_ports)
{
    ...
    {
     //   DEBUG_WRAP(DebugMessage(DEBUG_DETECT, "   Src->Src check failed, trying inverse test\n"););
        if(CheckAddrPort(rtn_idx->dip, CHECK_ADDR_DST_ARGS(rtn_idx), p,
                         rtn_idx->flags, CHECK_SRC_IP | INVERSE | (check_ports ? CHECK_SRC_PORT : 0)))
        {
          //  DEBUG_WRAP(DebugMessage(DEBUG_DETECT, "   Dst->Src check passed\n"););

            if(!CheckAddrPort(rtn_idx->sip, CHECK_ADDR_SRC_ARGS(rtn_idx), p,
                        rtn_idx->flags, CHECK_DST_IP | INVERSE | (check_ports ? CHECK_DST_PORT : 0)))
            {
            //    DEBUG_WRAP(DebugMessage(DEBUG_DETECT, "   Src->Dst check failed\n"););
                return 0;
            }
        }
        else
