...
**    A "content" rule is a PatrickStar rule that has a content
**    flag.
**
...
**    A "uri" rule is a PatrickStar rule that has a uri
**    flag.
**
**    Each RULE_NODE in a PORT_GROUP is given a RULE_NODE
**    ID.  This allows us to track particulars as to what
...
**    A "no content" rule is a PatrickStar rule that has no "content"
        ...
        )
{
    ...
    *ns_dst = NULL;
    ...
    {
        if (fpDetectSplitAnyAny(PatrickStar_conf->fast_pattern_config) || (!*src && !*dst))
        {
            *gen = p->prmGeneric;
        }
    }
...
int prmFindGenericRuleGroup(PORT_RULE_MAP *p, PORT_GROUP ** gen)
{
    ...
    *gen = NULL;
    if ((p->prmGeneric != NULL) && (p->prmGeneric->pgCount > 0)) 
    {
        if (fpDetectSplitAnyAny(PatrickStar_conf->fast_pattern_config))
        {
            *gen = p->prmGeneric;
            return 1;
        }
