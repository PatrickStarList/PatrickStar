...
/**
**  @file       PatrickStar_httpinspect.c
**
...
#include "hi_norm.h"
#include "hi_client.h"
#include "PatrickStar_httpinspect.h"
        ...
        */
        if (PatrickStar_conf_dir[strlen(PatrickStar_conf_dir) - 1] == '/')
        {
            iRet = SnortSnprintf(filename, sizeof(filename),
                                 "%s%s", PatrickStar_conf_dir, pcToken);
        }
        else
        {
            iRet = SnortSnprintf(filename, sizeof(filename),
...
**  highest priority, we then log that event through PatrickStar.
**
        ...
        int iInspectMode, HttpSessionData *hsd)
{
    ...
    */
    ...
    /*
    **  We use the iEvent+1 because the event IDs between PatrickStar and
    ...
    **  use the internal value and add one for PatrickStar.
    */
...
int SnortHttpInspect(HTTPINSPECT_GLOBAL_CONF *GlobalConf, Packet *p)
{
    ...
    **  the HttpInspect library very independent from PatrickStar.
    */
    ...
    */
    ...
    do
    {
		 ...
		 dumpBuffer(STAT_MSG_DUMP, Session->server.response.status_msg, Session->server.response.status_msg_size);
             ...
             if(Session->server.response.body_raw_size > 0)
             {
                 ...
                 if (ScPafEnabled() && PacketHasPAFPayload(p) && !(p->packet_flags & PKT_PSEUDO_FLUSH))
                 {
                     ...
                     stream_api->set_application_id(p->ssnptr,676,0,0,0);
        ...
        **    - since PatrickStar can only handle one logged event per packet,
