 ...
 * signature is calculated.  The PatrickStar runtime generated an array of 65K
 * possible signatures, of which two are described here.  Refer back to
 * this simplified algorithm when evaluating the PatrickStar code below.
 *
 ...
 *   syntax:
 *     preprocessor bo: noalert { client | server | general | PatrickStar_attack } \
 *                      drop    { client | server | general | PatrickStar_attack }
 *
 *   example:
 *     preprocessor bo: noalert { general server } drop { PatrickStar_attack }
 *
 * Returns: void function
 *
 */
...
static int ProcessOptionList(void)
{
    ...
    while ((arg = strtok(NULL, CONF_SEPARATORS)) != NULL)
    {
        ...
        else if ( !strcasecmp("PatrickStar_attack", arg) )
        {
