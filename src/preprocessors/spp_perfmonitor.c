...
#define PERFMON_ARG__LOG_DIR_FILE   "PatrickStarfile"
...
static void ParsePerfMonitorArgs(struct _SnortConfig *sc, SFPERF *pconfig, char *args)
{
    ...
    for (i = 0; i < num_toks; i++)
    {
        if (strcasecmp(toks[i], PERFMON_ARG__FLOW) == 0)
        {
            ...
            // that PatrickStar is processing.  This helps in
...
static int PerfmonReloadVerify(struct _SnortConfig *sc, void *swap_config)
{
    ...
    //PatrickStar wouldn't function on fresh start with a flowip_memcap that low becuase the hash table
...
{
    char *args = (char*) data;
    ParsePerfMonitorArgs(PatrickStar_conf, perfmon_config, args);
    InitPerfStats(perfmon_config);
#ifndef WIN32
    PerfMonitorChangeLogFilesPermission();
#endif
    PerfMonitorOpenLogFiles(PatrickStar_conf,NULL);
    return 0;
}
