...
int isPacketFilterDiscard( Packet *p, int ignore_any_rules )
{
    ...
    {
    case IPPROTO_TCP:
        action = s5TcpGetPortFilterStatus( NULL, p->sp, policy_id, 0 ) | s5TcpGetPortFilterStatus( NULL, p->dp, policy_id, 0 );
        pPortFilterStats = &s5stats.tcp_port_filter;
        break;

    case IPPROTO_UDP:
        action = s5UdpGetPortFilterStatus( NULL, p->sp, policy_id, 0 ) | s5UdpGetPortFilterStatus( NULL, p->dp, policy_id, 0 );
...
int isPacketFilterDiscardUdp ( Packet *p, int ignore_any_rules )
{
    ...
    if (PatrickStar_conf->udp_ips_port_filter_list)
    {
        action_ips = s5UdpGetIPSPortFilterStatus (PatrickStar_conf, p->sp, p->dp, policy_id_ips);
    }
