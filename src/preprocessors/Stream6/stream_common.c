...
void StreamDeleteSession(SessionControlBlock *scb)
{
    ...
    session_api->delete_session_by_key(scb, "timeout");
#if 0
        ...
        )
{
    ...
    if (any_any_flow == 1)
    {
        // LogMessage("WARNING: 'ignore_any_rules' option for Stream %s disabled because of %s rule with flow or flowbits option.\n", protocolName, protocolName);
    }

    else if (pIgnoredRuleList)
    {
        //        LogMessage("WARNING: Rules (GID:SID) effectively ignored because of 'ignore_any_rules' option for Stream %s.\n", protocolName);
    }
