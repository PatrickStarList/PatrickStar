...
int default_ports[] =
{
    21, 23, 25, 42, 80, 110, 111, 137, 139, 143, 445, 513, 514
};
...
{
   "ftp", "telnet", "smtp", "http", "pop3", "dcerpc", "netbios-ssn", "imap"
};
#endif
...
static inline void InitFlushPointList(FlushPointList *flush_point_list, uint32_t value, uint32_t range, int use_static)
{
    ...
    if (!flush_point_list->initialized)
    {
        ...
        {
            if (PatrickStar_conf->run_flags & RUN_FLAG__STATIC_HASH)
            {
                if ( i == 0 )
        ...
        uint16_t port, bool c2s, uint8_t flush_policy, bool all_services)
{
    ...
    InitFlushMgr(PatrickStar_conf, &pst->flush_mgr,
        ...
        )
{
    ...
    for (i = 0; i < pPolicyConfig->tcp_config->num_policies; i++)
    {
        ...
        for (j = 0; j < MAX_PORTS; j++)
        {
            ...
            InitFlushMgr(PatrickStar_conf, client, fpl, flush_policy, 0);

            server = &policy->flush_config[j].server;
            flush_policy = policy->flush_config[j].server.flush_policy;
            InitFlushMgr(PatrickStar_conf, server, fpl, flush_policy, 0);
        }
#ifdef TARGET_BASED
...
void **StreamGetPAFUserDataTcp( SessionControlBlock* scb, bool to_server, uint8_t id )
{
    ...
    if (!id || !scb->proto_specific_data || !scb->proto_specific_data->data)
        return NULL;

    tcpssn = ( TcpSession * ) scb->proto_specific_data->data;
    ps = ( to_server ) ? &tcpssn->server.paf_state : &tcpssn->client.paf_state;
...
bool StreamIsPafActiveTcp( SessionControlBlock *scb, bool to_server )
{
    ...
    if( !scb->proto_specific_data || !scb->proto_specific_data->data)
        return false;
...
bool StreamActivatePafTcp (SessionControlBlock *scb, int dir, int16_t service_port, uint8_t type)
{
    ...
    if( !scb->proto_specific_data || !scb->proto_specific_data->data || !ScPafEnabled())
        return false;
...
void StreamInitTcp( void )
{
    ...
    pkt_snaplen = (PatrickStar_conf->pkt_snaplen > 0) ? PatrickStar_conf->pkt_snaplen : PKT_SNAPLEN;
}
...
{
    if ( !InternalEventIsEnabled(PatrickStar_conf->rate_filter_config, eventSid) )
        return;
...
{
    NormMode mode = Normalize_GetMode(PatrickStar_conf, NORM_TCP_BLOCK);
...
{
    uint8_t* opt;
    NormMode mode =  Normalize_GetMode(PatrickStar_conf, NORM_TCP_OPT);
...
{
    NormMode mode = Normalize_GetMode(PatrickStar_conf, NORM_TCP_TRIM_SYN);
...
{
    NormMode mode = Normalize_GetMode(PatrickStar_conf, NORM_TCP_TRIM_RST);
    if ( mode != NORM_MODE_OFF && p->dsize > max )
    {
...
{
    NormMode mode = Normalize_GetMode(PatrickStar_conf, NORM_TCP_TRIM_WIN);
    if ( mode != NORM_MODE_OFF && p->dsize > max )
    {
...
{
    NormMode mode = Normalize_GetMode(PatrickStar_conf, NORM_TCP_TRIM_MSS);
    if ( mode != NORM_MODE_OFF && p->dsize > max )
    {
...
{
    NormMode mode = Normalize_GetMode(PatrickStar_conf, NORM_TCP_ECN_STR);
    if ( mode != NORM_MODE_OFF )
    {
        ...
        Packet *p, int *eventcode, int *got_ts)
{
    ...
    if ( p->tcph->th_flags & TH_ACK &&
            Normalize_GetMode(PatrickStar_conf, NORM_TCP_OPT) != NORM_MODE_OFF)
    {
...
static inline void UpdateSsn(Packet* p, StreamTracker *rcv, StreamTracker *snd, TcpDataBlock *tdb)
{
    ...
    if (
        ...
        rcv->s_mgr.state == TCP_STATE_ESTABLISHED &&
            ...
            Normalize_GetMode(PatrickStar_conf, NORM_TCP_IPS) )
            {
        ...
        Packet *p, sfaddr_t* sip, sfaddr_t* dip, uint16_t sp, uint16_t dp, uint32_t dir )
{
    ...
    if ( st->tcp_policy )
        UpdateFlushMgr(PatrickStar_conf, &st->flush_mgr, &st->tcp_policy->flush_point_list, st->flags);
        ...
        sfaddr_t* sip, sfaddr_t* dip, uint16_t sp, uint16_t dp, uint32_t dir)
{
    ...
    do
    {
        ...
        STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "Attempting to flush %lu bytes\n", footprint););
        ...
        sfaddr_t* sip, sfaddr_t* dip, uint16_t sp, uint16_t dp, uint32_t dir)
{
    FlushMgr* fm;
#ifdef NORMALIZER
    if ( Stream_NormGetMode(st->reassembly_policy, PatrickStar_conf, NORM_TCP_IPS) == NORM_MODE_ON )
    {
...
static void FlushQueuedSegs(SessionControlBlock *scb, TcpSession *tcpssn)
{
    ...
    if (tcpssn->client.seglist && !(scb->ha_state.ignore_direction & SSN_DIR_FROM_SERVER) )
    {
        ...
        if( scb->session_config != session_configuration )
        {
            ...
            p->configPolicyId = PatrickStar_conf->targeted_policies[scb->ipsPolicyId]->configPolicyId;
        ...
        {
            SnortEventqPush();
            SnortEventqLog(PatrickStar_conf->event_queue, p);
            SnortEventqReset();
            SnortEventqPop();
        }
        tcpssn->client.flags &= ~TF_FORCE_FLUSH;
    ...
    if( !scb->session_established )
    {
        if( s5TcpPolicy->flags & STREAM_CONFIG_REQUIRE_3WHS )
        {
            ...
            else
            {
                 ...
                 * connections that already existed before PatrickStar started.
                 */
...
midstream_pickup_allowed:
            if (p->dsize || TCP_ISFLAGSET(p->tcph, (TH_SYN|TH_ACK)))
            {
...
static uint32_t StreamGetTcpTimestamp( Packet *p, uint32_t *ts, int strip )
{
    ...
    while(i < p->tcp_option_count && i < TCP_OPTLENMAX)
    {
        ...
        {
#ifdef NORMALIZER
            if ( strip && Normalize_GetMode(PatrickStar_conf, NORM_TCP_OPT) == NORM_MODE_ON )
            {
...
static void FinishServerInit(Packet *p, TcpDataBlock *tdb, TcpSession *ssn)
{
  ...
  //  STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "seglist_base_seq = %X\n", client->seglist_base_seq););
        ...
        StreamSegment **retSeg)
{
    ...
    /* handle the urg ptr */
    if ((p->tcph->th_flags & TH_URG) && (ntohs(p->tcph->th_urp) < p->dsize))
    {
        switch(st->os_policy)
        {
        case STREAM_POLICY_LINUX:
        ...
        StreamSegment **retSeg)
{
 ...
 //   STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "added %d bytes on segment list @ seq: 0x%X, total %lu, " "%d segments queued\n", ss->size, ss->seq, st->seg_bytes_logical, SegsToFlush(st, 0)););
        ...
        TcpSession *tcpssn)
{
...
#ifdef NORMALIZER
    ips_data = Stream_NormGetMode(st->reassembly_policy, PatrickStar_conf, NORM_TCP_IPS);
        ...
        if(overlap > 0)
        {
            ...
            switch(reassembly_policy)
            {
                ...
                case REASSEMBLY_POLICY_MACOS:
                  //  STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "left overlap, honoring old data\n"););
#ifdef NORMALIZER
              ...
              //      STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "left overlap, honoring new data\n"););
                    break;
                case REASSEMBLY_POLICY_LAST:
                    /* True "Last" policy" */
    ...
    while(right && !done && SEQ_LT(right->seq, seq_end))
    {
        ...
        {
          //  STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "ftp-data daq retry or rexmit, do not call RetransmitHandle as \ data beyond this packet is already received\n"););
        }
        else
        {
        ...
        TcpDataBlock *tdb, StreamTcpPolicy *s5TcpPolicy)
{
  ...
  //  if (!IsTCPFastOpenPkt(p) && (p->tcph->th_flags & TH_SYN) && (listener->os_policy != STREAM_POLICY_MACOS))
    if (!IsTCPFastOpenPkt(p) && (p->tcph->th_flags & TH_SYN))
    {
    ...
    if((tdb->seq == listener->r_nxt_ack) || (p->tcph->th_flags & TH_SYN))
    {
        ...
        if((listener->s_mgr.state_queue == TCP_STATE_CLOSE_WAIT) &&
                (Stream_NormGetMode(listener->reassembly_policy, PatrickStar_conf, NORM_TCP_IPS) == NORM_MODE_ON))
        {
        ...
        TcpDataBlock *tdb, TcpSession *tcpssn)
{
    switch (listener->os_policy)
    {
        ...
        case STREAM_POLICY_VISTA:
            ...
            {
              //  STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "Got syn on established windows ssn, which causes Reset," "bailing\n"););
        ...
        StreamTcpPolicy *s5TcpPolicy, SFXHASH_NODE *hash_node)
{
    ...
    {
      //  STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "Lightweight session not TCP on TCP packet\n"););
        return retcode;
    }
    ...
    if (!IsTCPFastOpenPkt(p) && p->dsize && TCP_ISFLAGSET(p->tcph, TH_SYN))
    {
                ...
                STREAM_POLICY_MACOS)
        {
            ...
            if ( Normalize_GetMode(PatrickStar_conf, NORM_TCP_TRIM_SYN) == NORM_MODE_OFF)
#endif
            {
                //EventDataOnSyn(s5TcpPolicy);
            ...
            (TCP_STATE_LISTEN == talker->s_mgr.state) )
    {
        if(p->tcph->th_flags & TH_ACK)
        {
            ...
            {
              //  STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "Pkt ack is out of bounds, bailing!\n"););
                Discard();
                NormalTrimPayloadIfWin(p, 0, tdb);
...
#ifdef NORMALIZER
            if ( Normalize_GetMode(PatrickStar_conf, NORM_TCP_IPS) == NORM_MODE_ON )
                listener->s_mgr.state = TCP_STATE_CLOSED;
            /* else for ids:
               leave listener open, data may be in transit */
#endif
    ...
    if(got_ts && SEQ_EQ(listener->r_win_base, tdb->seq))
    {
        if((int32_t)(tdb->ts - talker->ts_last) >= 0 ||
                (uint32_t)p->pkth->ts.tv_sec >= talker->ts_last_pkt+PAWS_24DAYS)
        {
            STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "updating timestamps...\n"););
            talker->ts_last = tdb->ts;
            talker->ts_last_pkt = p->pkth->ts.tv_sec;
        }
    }
...
//    if ( s5TcpPolicy->policy != STREAM_POLICY_NOACK )
    {
        if (listener->tcp_policy->max_window && (tdb->win > listener->tcp_policy->max_window))
        {
            ...
            return retcode | ACTION_BAD_PKT;
                ...
                && !(scb->ha_state.session_flags & SSNFLAG_MIDSTREAM))
        {
            STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "Window slammed shut!\n"););
...
//                STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "listener state is SYN_SENT...\n"););
                if(IsBetween(listener->l_unackd, listener->l_nxt_seq, tdb->ack) &&
            ...
            case TCP_STATE_FIN_WAIT_1:
                ...
                {
                    UpdateSsn(p, listener, talker, tdb);
//                    STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "bad ack!\n"););
                }
                break;
                ...
                (listener->flush_mgr.flush_policy == STREAM_FLPOLICY_IGNORE))
            {
                        ...
                        !(talker->s_mgr.sub_state & SUB_FIN_SENT) )
                {
                    ...
                    if ((listener->flush_mgr.flush_policy != STREAM_FLPOLICY_PROTOCOL) &&
                            ...
                            Normalize_GetMode(PatrickStar_conf, NORM_TCP_IPS) == NORM_MODE_ON)
                    {
                        ...
                        (talker->s_mgr.state == TCP_STATE_LAST_ACK)))
                {
                    ...
                    {
//                        STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "FIN beyond previous, ignoring\n"););
            ...
            else
            {
                ...
                {
//                    STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "FIN inside transition_seq, bailing\n"););
                    goto dupfin;
                }
                if ((listener->s_mgr.state_queue == TCP_STATE_CLOSE_WAIT) ||
                        (talker->s_mgr.state == TCP_STATE_FIN_WAIT_1) ||
...
dupfin:
    ...
    {
        //dropssn:
     //   STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "Session terminating, flushing session buffers\n"););
        ...
        if(p->packet_flags & PKT_FROM_SERVER)
        {
//            STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "flushing FROM_SERVER\n"););
        ...
        {
         //   STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "flushing FROM_CLIENT\n"););
            if(listener->seg_bytes_logical)
            {
                uint32_t flushed = flush_stream(tcpssn, listener, p,
                        GET_SRC_IP(p), GET_DST_IP(p),
                                       ...
                                       StreamTracker *trk, Packet *pkt, uint64_t *flags )
{
    ...
    while ( seg && *flags && (total < avail) )
    {
       ...
       //     STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "flush_pdu_ips and purge\n"););
                                        ...
                                        StreamTracker* trk, Packet* pkt, uint64_t* flags)
{
    ...
    while ( seg && *flags && SEQ_LT(seg->seq, trk->r_win_base) )
    {
        ...
        if (*flags & PKT_PURGE)
        {
            ...
            //Set the seglist value to SL_BUF_FLUSHED
//            STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "Flag PKT_PURGE is set"););
            seg->buffered = SL_BUF_FLUSHED;
            return 0;
        }
        if ( flush_pt > 0 )
...
static int StreamSeglistDeleteNode (StreamTracker* st, StreamSegment* seg)
{
 ...
 //   STREAM_DEBUG_WRAP( DebugMessage(DEBUG_STREAM_STATE, "Dropping segment at seq %X, len %d\n", seg->seq, seg->size););
        ...
        StreamTracker* st, StreamSegment* seg, uint32_t flush_seq)
{
    ...
    if( (seg->seq + seg->size - seg->urg_offset) > flush_seq )
    {
        ...
        if( s5_paf_active(&st->paf_state) )
        {
             ...
             if ( delta < seg->size )
             {
     //             STREAM_DEBUG_WRAP( DebugMessage(DEBUG_STREAM_STATE, "Left-Trimming segment at seq %X, len %d, delta %u\n", seg->seq, seg->size, delta););
...
void StreamSetFlushPointTcp(SessionControlBlock *scb, char dir, uint32_t flush_point)
{
    ...
    if (!scb || !flush_point || !scb->proto_specific_data || !scb->proto_specific_data->data)
        return ; 
...
char StreamSetReassemblyTcp(SessionControlBlock *scb, uint8_t flush_policy, char dir, char flags)
{
    ...
    {
        if ((dir & SSN_DIR_TO_SERVER) && (tcpssn->server.flush_mgr.flush_policy == STREAM_FLPOLICY_NONE))
        {
            InitFlushMgr(PatrickStar_conf, &tcpssn->server.flush_mgr,
                    &tcpssn->server.tcp_policy->flush_point_list,
                    flush_policy, 0);
            tcpssn->server.flags |= tmp_flags;
        }

        if ((dir & SSN_DIR_TO_CLIENT) && (tcpssn->client.flush_mgr.flush_policy == STREAM_FLPOLICY_NONE))
        {
            InitFlushMgr(PatrickStar_conf, &tcpssn->client.flush_mgr,
    ...
    else if (flags & STREAM_FLPOLICY_SET_ABSOLUTE)
    {
        if (dir & SSN_DIR_TO_SERVER)
        {
            ...
            if ((tcpssn->server.flush_mgr.flush_policy == STREAM_FLPOLICY_FOOTPRINT_IPS)
                    && session_api->get_application_data(scb, PP_FTPTELNET))
            {
...
char StreamIsStreamSequencedTcp(SessionControlBlock *scb, char dir)
{
    ...
    tcpssn = (TcpSession *)scb->proto_specific_data->data;
    if ((dir & SSN_DIR_FROM_CLIENT) && (tcpssn->server.flags & TF_MISSING_PREV_PKT))
    {
        return 0;
    }

    if ((dir & SSN_DIR_FROM_SERVER) && (tcpssn->client.flags & TF_MISSING_PREV_PKT))
    {
        return 0;
    }
...
int StreamMissingInReassembledTcp(SessionControlBlock *scb, char dir)
{
    ...
    else if ((dir & SSN_DIR_FROM_SERVER) && (tcpssn->client.flags & TF_MISSING_PREV_PKT))
    {
        return SSN_MISSING_BEFORE;
    }
...
char StreamPacketsMissingTcp(SessionControlBlock *scb, char dir)
{
    ...
    tcpssn = (TcpSession *)scb->proto_specific_data->data;
    if ((dir & SSN_DIR_FROM_CLIENT) && (tcpssn->server.flags & TF_PKT_MISSED))
    {
        return 1;
    }

    if ((dir & SSN_DIR_FROM_SERVER) && (tcpssn->client.flags & TF_PKT_MISSED))
    {
        return 1;
    }
...
static void targetPolicyIterate(void (*callback)(int))
{
    ...
    for (i = 0; i < PatrickStar_conf->num_policies_allocated; i++)
    {
        if (PatrickStar_conf->targeted_policies[i] != NULL)
        {
            callback(i);
        }
    }
...
static void policyDecoderFlagsSaveNClear(int policyId)
{
    SnortPolicy *pPolicy = PatrickStar_conf->targeted_policies[policyId];
...
int StreamSimulatePeerTcpAckp( SessionControlBlock *scb, uint8_t dir, uint32_t tcp_payload_len )
{
    ...
    {
        //STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "Stream: no SCB %p or zero length %d \n", scb, tcp_payload_len););
        return -1;
    }
    if (scb->proto_specific_data)
        tcpssn = (TcpSession *)scb->proto_specific_data->data;
    if (!tcpssn) 
    {
      //  STREAM_DEBUG_WRAP(DebugMessage(DEBUG_STREAM_STATE, "Stream: no tcpssn \n"););
        return -1;
    }
