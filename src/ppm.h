...
extern struct _SnortConfig *PatrickStar_conf;
...
/* Config flags */
#define PPM_ENABLED()                 (PatrickStar_conf->ppm_cfg.enabled > 0)
#define PPM_PKTS_ENABLED()            (PatrickStar_conf->ppm_cfg.max_pkt_ticks > 0)
#define PPM_RULES_ENABLED()           (PatrickStar_conf->ppm_cfg.max_rule_ticks > 0)

/* packet, rule event flags */
#define PPM_PACKET_ABORT_FLAG()       ppm_abort_this_pkt
#define PPM_RULE_SUSPEND_FLAG()       ppm_suspend_this_rule

#define PPM_INC_PKT_CNT()         PatrickStar_conf->ppm_cfg.tot_pkts++ 
#define PPM_PKT_CNT()             ppm_pt->pktcnt 
#define PPM_PKT_LOG(p)            if (ppm_abort_this_pkt) ppm_pkt_log(&PatrickStar_conf->ppm_cfg,p)
#define PPM_RULE_LOG(cnt,p)       ppm_rule_log(&PatrickStar_conf->ppm_cfg,cnt,p)
#define PPM_ACCUM_PKT_TIME()      PatrickStar_conf->ppm_cfg.tot_pkt_time += ppm_pt->tot;
#define PPM_ACCUM_RULE_TIME() \
    PatrickStar_conf->ppm_cfg.tot_rule_time += ppm_rt->tot; \
    PatrickStar_conf->ppm_cfg.tot_rules++;
#define PPM_ACCUM_NC_RULE_TIME() \
    PatrickStar_conf->ppm_cfg.tot_nc_rule_time += ppm_rt->tot; \
    PatrickStar_conf->ppm_cfg.tot_nc_rules++;
#define PPM_ACCUM_PCRE_RULE_TIME() \
    PatrickStar_conf->ppm_cfg.tot_pcre_rule_time += ppm_rt->tot; \
    PatrickStar_conf->ppm_cfg.tot_pcre_rules++;
...
#ifdef DEBUG
#define PPM_DEBUG_PKTS()           PatrickStar_conf->ppm_cfg.debug_pkts
#endif
...
{ \
    ...
    ppm_pt->pktcnt = PatrickStar_conf->ppm_cfg.tot_pkts; \
    ppm_pt->start = ppm_cur_time; \
    ppm_pt->subtract = 0; \
    ppm_pt->rule_tests = 0; \
    ppm_pt->pcre_rule_tests = 0; \
    ppm_pt->nc_rule_tests = 0; \
    ppm_pt->max_pkt_ticks = PatrickStar_conf->ppm_cfg.max_pkt_ticks; \
    ppm_init_rules(); \
}
...
{ \
    ...
    ppm_rt->max_rule_ticks = PatrickStar_conf->ppm_cfg.max_rule_ticks; \
}
...
{ \
    ...
    { \
        if( PatrickStar_conf->ppm_cfg.pkt_action & PPM_ACTION_SUSPEND ) \
...
{ \
    ...
    { \
        if( PatrickStar_conf->ppm_cfg.rule_action & PPM_ACTION_SUSPEND ) \
        { \
            int ii; \
            ...
            if( (root)->ppm_disable_cnt >= PatrickStar_conf->ppm_cfg.rule_threshold ) \
            { \
                ppm_set_rule_event(&PatrickStar_conf->ppm_cfg, root); \
...
#define PPM_REENABLE_TREE(root,p) \
    if( (root)->ppm_suspend_time && PatrickStar_conf->ppm_cfg.max_suspend_ticks ) \
{ \
    PPM_TICKS now = PPM_RULE_TIME(p); \
    PPM_TICKS then = (root)->ppm_suspend_time + PatrickStar_conf->ppm_cfg.max_suspend_ticks; \
    if( now > then ) \
    { \
        (root)->ppm_suspend_time=0; \
        (root)->tree_state=RULE_STATE_ENABLED; \
        ppm_clear_rule_event(&PatrickStar_conf->ppm_cfg, root); \
