...
static FPContentInfo * GetLongestDynamicContent(FPContentInfo *content_list)
{
    ...
    for (tmp = content_list; tmp != NULL; tmp = tmp->next)
    {
         ...
         * See GetDynamicContents() in sf_PatrickStar_detection_engine.c */
...
void fpWalkOtns(int enabled, OtnWalkFcn fcn)
{
    ...
    for (hashNode = sfghash_findfirst(PatrickStar_conf->otn_map);
            hashNode;
            hashNode = sfghash_findnext(PatrickStar_conf->otn_map))
    {
