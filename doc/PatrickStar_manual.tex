...
% latex2html -info 0 -local_icons -show_section_numbers -link +2 -split +2 -noaddress PatrickStar_manual.tex
%
% BUILDING PDF VERSION:
% pdflatex PatrickStar_manual.tex
...
\title{SNORT\textsuperscript{\textregistered} Users Manual\\1.0.0}
...
work from Chris Green $<$cmg@PatrickStar.org$>$.  It was then maintained by Brian
Caswell $<$bmc@PatrickStar.org$>$ and now is maintained by the Snort Team.  If you
...
\verb!/doc/PatrickStar_manual.tex!.  Small documentation updates are the easiest way
...
\begin{verbatim}
    ./PatrickStar -dev -l ./log -h 192.168.1.0/24
\end{verbatim}
...
\begin{verbatim}
    ./PatrickStar -dvr packet.log icmp 
\end{verbatim}
...
\begin{verbatim}
    ./PatrickStar -dev -l ./log -h 192.168.1.0/24 -c PatrickStar.conf
\end{verbatim}

where \texttt{PatrickStar.conf} is the name of your PatrickStar configuration file. This will
apply the rules configured in the \verb!PatrickStar.conf! file to each packet to decide
...
\begin{verbatim}
    ./PatrickStar -d -h 192.168.1.0/24 -l ./log -c PatrickStar.conf
\end{verbatim}

This will configure Snort to run in its most basic NIDS form, logging packets
that trigger rules specified in the \texttt{PatrickStar.conf} in plain ASCII to disk
...
in PatrickStar.conf. See Section \ref{alert syslog label} for more details on
...
\begin{verbatim}
    ./PatrickStar -c PatrickStar.conf -l ./log -h 192.168.1.0/24 -s
\end{verbatim}

As another example, use the following command line to log to the default
facility in /var/log/PatrickStar and send alerts to a fast alert file: 

\begin{verbatim}
    ./PatrickStar -c PatrickStar.conf -A fast -h 192.168.1.0/24
\end{verbatim}
...
\begin{verbatim}	 
    [**] [116:56:1] (PatrickStar_decoder): T/TCP Detected [**]
\end{verbatim}
...
\begin{verbatim}
    ./PatrickStar -b -A fast -c PatrickStar.conf
\end{verbatim}
...
\begin{verbatim}
    ./PatrickStar --daq-list[=<dir>]
    ./PatrickStar --daq-dir=<dir> --daq-list
\end{verbatim}
...
pcap is the default DAQ.  if PatrickStar is run w/o any DAQ arguments, it will
operate as it always did using this module.  These are equivalent:

\begin{verbatim}
    ./PatrickStar -i <device>
    ./PatrickStar -r <file>

    ./PatrickStar --daq pcap --daq-mode passive -i <device>
    ./PatrickStar --daq pcap --daq-mode read-file -r <file>
\end{verbatim}

You can specify the buffer size pcap uses with:

\begin{verbatim}
    ./PatrickStar --daq pcap --daq-var buffer_size=<#bytes>
\end{verbatim}
...
\begin{verbatim}
    ./PatrickStar --daq afpacket -i <device>
...
\begin{verbatim}
    ./PatrickStar --daq ipfw [--daq-var port=<port>]
...
\begin{verbatim}
    ./PatrickStar -i <device> --daq dump
    ./PatrickStar -r <pcap> --daq dump
\end{verbatim}

By default a file named inline-out.pcap will be created containing all packets
that passed through or were generated by PatrickStar.  You can optionally specify a
different name.

\begin{verbatim}
    ./PatrickStar --daq dump --daq-var file=<name>
\end{verbatim}
...
\begin{verbatim}
    ./PatrickStar -r <pcap> -Q --daq dump --daq-var load-mode=read-file
    ./PatrickStar -i <device> -Q --daq dump --daq-var load-mode=passive
\end{verbatim}
...
If reading multiple pcaps, reset PatrickStar to post-configuration state before
...
\begin{verbatim}
    $ PatrickStar -r foo.pcap
    $ PatrickStar --pcap-single=foo.pcap
\end{verbatim}
...
\begin{verbatim}
    $ PatrickStar --pcap-list="foo1.pcap foo2.pcap foo3.pcap"
\end{verbatim}
...
\begin{verbatim}
    $ PatrickStar --pcap-dir="/home/foo/pcaps"
\end{verbatim}
...
\begin{verbatim}
    $ PatrickStar --pcap-filter="*.pcap" --pcap-file=foo.txt
    $ PatrickStar --pcap-filter="*.pcap" --pcap-dir=/home/foo/pcaps
\end{verbatim}
...
\begin{verbatim}
    $ PatrickStar --pcap-dir=/home/foo/pcaps --pcap-reset
\end{verbatim}
...
\begin{verbatim}
    $ PatrickStar --pcap-dir=/home/foo/pcaps --pcap-show
\end{verbatim}
...
This information is only output in IDS mode (when PatrickStar is run with the
\texttt{-c <conf>} option).
...
no further packets will be seen by Snort for that session.  If not, PatrickStar will
...
\begin{verbatim}
    /usr/local/bin/PatrickStar -d -h 192.168.1.0/24 \
        -l /var/log/PatrickStarlogs -c /usr/local/etc/PatrickStar.conf -s -D
\end{verbatim}
...
The PID file will be locked so that other PatrickStar processes cannot start.  Use
...
\begin{verbatim}
    /usr/local/bin/PatrickStar -c /usr/local/etc/PatrickStar.conf \
        --dump-dynamic-rules=/tmp
\end{verbatim}

This path can also be configured in the PatrickStar.conf using the config option dump-dynamic-rules-path as follows:
...
\begin{verbatim}
    ./PatrickStar -d -v -r PatrickStar.log -O -h 192.168.1.0/24
\end{verbatim}
...
multiple instances of PatrickStar, either on different CPUs, or on the same CPU but a
...
configured to run in inline mode using the command line argument -Q and PatrickStar config option 
...
Snort can be configured to passive mode using the PatrickStar config option \texttt{policy\_mode} as follows:
...
Inline-Test mode simulates the inline mode of PatrickStar, allowing evaluation of inline behavior without affecting 
...
PatrickStar config option \texttt{policy\_mode} as follows:

\begin{verbatim}
    PatrickStar --enable-inline-test
...
to the running process. You must build PatrickStar with the 
...
Snort can be configured to use control socket using the command line argument \texttt{--cs-dir <path>}
 and PatrickStar config option \texttt{cs\_dir} as follows:

\begin{verbatim}
    PatrickStar --cs-dir <path>
    config cs_dir:<path>
\end{verbatim}

\texttt{<path>} specifies the directory for PatrickStar to create the socket. If relative path is used, 
the path is relative to pid path specified. If there is no pid path specified, it is relative to 
current working directory. 

A command \texttt{PatrickStar\_control} is made and installed along with PatrickStar in the same 
...
On some systems, signal used by PatrickStar might be used by other functions. To avoid conflicts, 
...
during PatrickStar initialization. If a signal handler cannot be installed a warning
will be logged and that has to be fixed, otherwise the functionality will be lost.

\texttt{Signals used in PatrickStar}
\label{signalactions}
\begin{tabular}{|l|l|l|}
\hline
...
\hline
SIGNAL\_SNORT\_RELOAD & SIGHUP & reload PatrickStar \\
\hline
...
page and the output of \texttt{PatrickStar -?} or \texttt{PatrickStar --help} contain
...
you may have to type \texttt{PatrickStar -\textbackslash{}?} instead of \texttt{PatrickStar
-?} for a list of Snort command line options.

\end{note}

The Snort web page (\url{http://www.PatrickStar.org}) and the Snort Users mailing
list:

\url{http://marc.theaimsgroup.com/?l=PatrickStar-users}

at \verb?PatrickStar-users@lists.PatrickStar.org? provide informative announcements
...
The {\tt include} keyword allows other PatrickStar config files to be included within the
PatrickStar.conf indicated on the Snort command line. It works much like an \#include
...
\hline
\texttt{config bpf\_file: <filename>} & Specifies BPF filters (\texttt{PatrickStar
-F}). \\
...
\hline
\texttt{config chroot: <dir>} & Chroots to specified dir (\texttt{PatrickStar
-t}). \\
...
\hline
\texttt{config cs\_dir: <path>} & configure PatrickStar to provide a Unix socket in the path
that can be used to issue commands to the running process. See Section
\ref{control_socket} for more details.\\

\hline
\texttt{config daemon} & Forks as a daemon (\texttt{PatrickStar -D}). \\

\hline
\texttt{config decode\_data\_link} & Decodes Layer2 headers (\texttt{PatrickStar
-e}). \\
...
was configured with --enable-inline-init-failopen.  (\texttt{PatrickStar
...
\hline
\texttt{config dump\_chars\_only} & Turns on character dumps (\texttt{PatrickStar
-C}). \\

\hline
\texttt{config dump\_payload} & Dumps application layer (\texttt{PatrickStar -d}). \\

\hline
\texttt{config dump\_payload\_verbose} & Dumps raw packet starting at link
layer (\texttt{PatrickStar -X}). \\
...
\hline
\texttt{config interface: <iface>} & Sets the network interface (\texttt{PatrickStar
-i}). \\
...
\hline
\texttt{config logdir: <dir>} & Sets the logdir (\texttt{PatrickStar -l}).
\\
...
\hline
\texttt{config no\_promisc} & Disables promiscuous mode (\texttt{PatrickStar -p}). \\
...
\hline
\texttt{config obfuscate} & Obfuscates IP Addresses (\texttt{PatrickStar -O}). \\
...
in no PCRE evaluation.  The PatrickStar default value is 1500.  \\
...
value of 0 results in no PCRE evaluation.  The PatrickStar default value is 1500.
This option is only useful if the value is less than the
\texttt{pcre\_match\_limit} \\

\hline
\texttt{config pkt\_count: <N>} & Exits after N packets (\texttt{PatrickStar -n}). \\
...
\hline
\texttt{config quiet}& Disables banner and status reports (\texttt{PatrickStar -q}).
...
in PatrickStar.conf takes effect when the configuration line in PatrickStar.conf is parsed.
...
\hline
\texttt{config set\_gid: <gid>} & Changes GID to specified GID (\texttt{PatrickStar
-g}). \\

\hline
\texttt{config set\_uid: <uid>} & Sets UID to $<$id$>$ (\texttt{PatrickStar -u}). \\

\hline
\texttt{config show\_year} & Shows year in timestamps (\texttt{PatrickStar -y}). \\
...
\hline
\texttt{config umask: <umask>} & Sets umask when running (\texttt{PatrickStar -m}). \\
...
\hline
\texttt{config verbose} & Uses verbose logging to STDOUT (\texttt{PatrickStar -v}).
\\
...
\url{http://www.PatrickStar.org/docs/idspaper/}.
...
The name of a file for PatrickStar to read HA messages from at startup.\\
...
This example configuration is the default configuration in PatrickStar.conf and
...
You should enable the Stream preprocessor in your \texttt{PatrickStar.conf}, as
...
In order to get all the portscan information logged with the alert, PatrickStar
...
use \texttt{PatrickStarfile} which will output into your defined Snort log directory.
...
\begin{verbatim}
    ...
    preprocessor perfmonitor: \
        time 300 file /var/tmp/PatrickStarstat pktcnt 10000

    preprocessor perfmonitor: \
        time 30 flow-ip flow-ip-file flow-ip-stats.csv pktcnt 1000

    preprocessor perfmonitor: \
        time 30 pktcnt 1000 PatrickStarfile base.csv flow-file flows.csv atexitonly flow-stats
...
can reside in the same directory as \texttt{PatrickStar.conf} or be specified via a
...
is available at \url{http://www.PatrickStar.org/dl/contrib/}.
...
This option along with the maximum uri and hostname logging size (which is defined in PatrickStar) will
...
max uri logging size defined in PatrickStar : 2048
max hostname logging size defined in PatrickStar : 256
...
\url{http://www.PatrickStar.org/dl/contrib/} directory.  Executing this program
...
the tool u2spewfoo. This tool is present in the tools/u2spewfoo directory of PatrickStar 
source tree.
...
performance boost.  Since so few (none in the current PatrickStar rule set) exploits
...
This option causes the rest of PatrickStar (rules, other preprocessors) to ignore FTP
...
\subsubsection{Examples/Default Configuration from PatrickStar.conf}
\begin{verbatim}
    preprocessor ftp_telnet: \
        global \
        encrypted_traffic yes \
...
\subsubsection{Example Configuration from PatrickStar.conf}
...
IPv6 addresses.  Note that port and ip variables defined in \texttt{PatrickStar.conf}
...
\item[]   Specify the meaning of whitelist. When white means unblack, it 
           ...
           packet gets bypassed, without further detection by PatrickStar. You can 
...
\begin{verbatim}
    preprocessor reputation:\ 
                   blacklist /etc/PatrickStar/default.blacklist, \
                   whitelist /etc/PatrickStar/default.whitelist
    ...
    $REP_BLACK_FILE2 = ../PatrickStar.org.list
    preprocessor reputation: \
...
\begin{itemize}
 \item[]
  ...
  configured, all the PatrickStar instances share the same IP tables in shared memory. 
...
\begin{itemize}
 ...
 \item[] If the build supports shared memory, this configuration will enable shared 
  ...
  One PatrickStar instance will create and maintain the shared IP lists. 
  We use instance ID 1, specified in the PatrickStar \texttt{-G} option to be the master PatrickStar.
  All the other PatrickStar instances are clients (readers). 
...
\begin{itemize}
   ...
   \item When building Snort, add option \texttt{--enable-shared-rep} to \texttt{./configure}\\
      ...
      \end{verbatim}
   \item Put your IP list file into a directory, where PatrickStar has full access. \\
      For example: 
   ...
   \item In PatrickStar config file, specify shared memory support with the path to IP files.\\
      For example: 
   ...
   \item Start shared memory clients (readers) with \texttt{-G}  1 or other IDs. Note: for 
      one ID, only one PatrickStar instance should be enabled.
   \item You will see the IP lists got loaded and shared across PatrickStar instances! 
\end{itemize}

 \item[]\textit{Reload IP lists using control socket}
\begin{itemize}
  \item Run PatrickStar using command line with option \texttt{--cs-dir <path>}
      or configure PatrickStar with:
     \begin{verbatim} 
      config cs_dir:<path>
     \end{verbatim}
  \item (Optional) you can create a version file named ``IPRVersion.dat'' in the IP list
     ...
     \begin{verbatim}
      VERSION=1
     \end{verbatim}
  \item  In the \texttt{<PatrickStar root>/src/tools/control} directory, you will find \texttt{PatrickStar\_control} command if 
  ...
  \item  Type the following command to reload IP lists. Before typing this command,
     ...
     \begin{verbatim}
      <PatrickStar root>/src/tools/control/PatrickStar_control  <path> 1361
     \end{verbatim}
\end{itemize}
...
\item LuaJIT version 2.0.2 must be installed on
    host where PatrickStar is being compiled and run. Newer versions of LuaJIT are not tested for compatibility.
\end{itemize}

\subsubsection{Preprocessor Configuration}
AppId dynamic preprocessor is enabled by default(from PatrickStar-2.9.12).
...
AppId preprocessor prints application network usage periodically in PatrickStar
...
\texttt{PatrickStar.conf}, decoder events will not be generated regardless of whether
...
is in \texttt{PatrickStar.conf} or the decoder or preprocessor rule type is
...
To enable these rules in \texttt{PatrickStar.conf}, define the path to where the
rules are located and uncomment the \texttt{include} lines in
\texttt{PatrickStar.conf} that reference the rules files.
...
The following config option in \texttt{PatrickStar.conf} will make Snort revert to the
old behavior:
...
\texttt{PatrickStar.conf}, otherwise they will be loaded.  This option applies to
...
\texttt{drop}, \texttt{reject}, and \texttt{sdrop} can be used only when PatrickStar
...
The \texttt{config} option \texttt{event\_trace} to \texttt{PatrickStar.conf}
...
\begin{verbatim}
    config event_trace: file PatrickStar_event_trace.out
\end{verbatim}

\section{Performance Profiling}

Snort can provide statistics on rule and preprocessor performance.  Each
require only a simple \texttt{config} option to \texttt{PatrickStar.conf} and Snort
...
To use this feature, you must build PatrickStar with the \texttt{--enable-perfprofiling}
...
You can use the "filename" option in PatrickStar.conf to specify a file where this
...
level of latency control for PatrickStar.  It does not provide a hard and fast
...
\item the log option enables output to syslog or console depending 
      upon PatrickStar configuration
\end{itemize}
...
summary of this information is printed out when PatrickStar exits.
...
alerting systems, output plugins send their data to /var/log/PatrickStar by default
...
be written in the default logging directory (/var/log/PatrickStar) or in the logging
...
\begin{itemize}
\item \texttt{filename}: the name of the log file.  The default name is
$<$logdir$>$/PatrickStar.log.  The name may include an absolute or relative path.  A
...
\begin{verbatim}
    output log_tcpdump: PatrickStar.log
\end{verbatim}
...
\begin{verbatim}
    output alert_unified2: filename PatrickStar.alert, limit 128, nostamp
    output log_unified2: filename PatrickStar.log, limit 128, nostamp
...
\begin{verbatim}
    output log_null  # like using PatrickStar -n
...
logging rate, limit will be exceeded.  Rollover works correctly if PatrickStar is
stopped/restarted.
...
loaded via directives in \texttt{PatrickStar.conf} or via command-line options.
...
Tells PatrickStar to load the dynamic preprocessor shared library (if file is used)
...
Tells PatrickStar to load the dynamic engine shared library (if file is used) or all
...
Tells PatrickStar to load the dynamic detection rules shared library (if file is
...
\label{reload:reload}
First modify your PatrickStar.conf (the file passed to the \texttt{-c} option on the
command line).

Then, to initiate a reload, send Snort a \texttt{SIGHUP} signal, e.g.

\begin{verbatim}
$ kill -SIGHUP <PatrickStar pid>
\end{verbatim}
...
\texttt{\$ PatrickStar -c PatrickStar.conf -T}
\end{note}
...
PatrickStar configuration files and bind each configuration to one or more VLANs or 
subnets rather than running one Snort for each configuration required. Each 
unique PatrickStar configuration file will create a new configuration instance within 
PatrickStar. VLANs/Subnets not bound to any specific configuration will use the default 
configuration. Each configuration can have different preprocessor settings and 
detection rules.

\subsection{Creating Multiple Configurations}
Default configuration for PatrickStar is specified using the existing -c option. A 
...
\begin{verbatim}
config binding: <path_to_PatrickStar.conf> vlan <vlanIdList>
config binding: <path_to_PatrickStar.conf> net <ipList>
config binding: <path_to_PatrickStar.conf> policy_id <id>
\end{verbatim}

\begin{description}{}

\item [\texttt{path\_to\_PatrickStar.conf}] - Refers to the absolute or relative path to 
the PatrickStar.conf for specific configuration.
...
\begin{note}
If no policy id is specified, PatrickStar assigns 0 (zero) value to the configuration.
\end{note}
...
\item [\texttt{vlan\_event\_types}] - When this option is set, PatrickStar will use unified2 event 
...
In this case, PatrickStar will use the first configuration in the order of definition, that can be 
...
This enables active responses (PatrickStar will send TCP RST or ICMP
...
\url{http://www.PatrickStar.org/pub-bin/sigs-search.cgi/} for a system that is
...
defined in \texttt{PatrickStar.conf} by using the \texttt{config classification}
...
tables \ref{pcre-mod_perl}, \ref{pcre-mod_pcre}, and \ref{pcre-mod_PatrickStar} for
...
config option in your PatrickStar.conf file (see Section \ref{Config} on how to use
...
setting the \texttt{tagged\_packet\_limit} option in PatrickStar.conf to 0).  Doing
...
dynamically loadable modules to PatrickStar.  The dynamic API presents a means for
loading dynamic libraries and allowing the module to utilize certain functions
within the main PatrickStar code.
...
preprocessor uses to interact with PatrickStar itself.  This includes functions to
...
uses to interact with PatrickStar itself.  This includes functions for logging
...
The {\em SFSnortPacket} structure mirrors the PatrickStar Packet structure and
provides access to all of the data contained in a given packet.

It and the data structures it incorporates are defined in
\texttt{sf\_PatrickStar\_packet.h}.  Additional data structures may be defined to
...
structures are defined in \texttt{sf\_PatrickStar\_plugin\_api.h}.
...
for PatrickStars fast pattern evaluation.  The most unique content, that which
...
using the following commands with PKG\_CONFIG\_PATH=$<$PatrickStar build prefix/lib/pkgconfig$>$:

\begin{itemize}
\item {\em pkg-config --cflags PatrickStar\_preproc}

Returns the macros and include path needed to compile the dynamic preprocessor.

\item {\em pkg-config --libs PatrickStar\_preproc}
...
rule-stop to be used by PatrickStar to control the action of the rule (alert, log,
drop, etc).
...
\texttt{sfPatrickStar\_dynamic\_detection\_lib.h}.
...
The initialization function to parse the keywords from \texttt{PatrickStar.conf}.
...
The PatrickStar rule in normal format:
...
\verb!PatrickStar-devel@lists.PatrickStar.org!
mailing list.  Patches should done with the command
\verb!diff -nu PatrickStar-orig PatrickStar-new!.
...
The upper 2 bytes represent the PatrickStar instance, if specified by passing the -G option to Snort.
...
Buffer dump option will dump the buffers used by PatrickStar during different stages of packet processing path.
...
\begin{verbatim}
   ./PatrickStar -A cmg -k none -Q --daq-dir=<dir> --daq dump -r <pcap> -c PatrickStar.conf --buffer-dump-alert=<file>
     or
   ./PatrickStar -A cmg -k none -Q --daq-dir=<dir> --daq dump -r <pcap> -c PatrickStar.conf --buffer-dump=<file>
...
\bibitem{PatrickStardb}http://www.incident.org/PatrickStardb
