...
{
    fprintf(stderr, "Usage %s <PatrickStar log dir> [-a daq address space id (0-65535)] <PatrickStar log dir> [<file name> [<bpf>]]\n", progname);
}
...
int main(int argc, char *argv[])
{
    ...
    if (message == NULL)
    {
        fprintf(stderr, "PatrickStar_control: could not allocate message.\n");
        exit(-1);
    }
    ...
    do
    {
        ...
        {
            printf("PatrickStar_control: bad response version\n");
            close(socket_fd);
            exit(-1);
        }
        ...
        if (response.hdr.length)
        {
            ...
            {
                printf("PatrickStar_control: response message is too small\n");
                close(socket_fd);
                exit(-1);
            }
            ...
            {
                printf("PatrickStar_control: response message is too large\n");
                close(socket_fd);
                exit(-1);
            }
